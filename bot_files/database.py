...
def set_exclusive_provider(provider_id):
    """تفعيل هذا المزود فقط وتعطيل كافة المزودين الآخرين لتجربة المزود حصرياً"""
    conn = get_db()
    c = conn.cursor()
    # تعطيل جميع المزودين وخفض أولويتهم
    c.execute("UPDATE providers SET enabled = 0, is_active = 0, priority = 50")
    # تفعيل المزود المختار فقط وإعطاؤه أولوية 100
    c.execute("UPDATE providers SET enabled = 1, is_active = 1, priority = 100 WHERE id = ?", (int(provider_id),))
    conn.commit()
    conn.close()
    add_provider_log(provider_id, f"Provider #{provider_id}", "set_exclusive", "success", "تم تعيين المزود كمزود وحيد نشط في النظام")
    return True
