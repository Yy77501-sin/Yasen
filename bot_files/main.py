...
TG_READY_BASE_URL = str(get_config_val('TG_READY_BASE_URL', get_config_val('TG_LION_BASE_URL', 'https://TG-Lion.net'))).strip()
...
def init_db():
    try:
        ...
        ''')
        for col_name, col_type in [
            ("product_title", "TEXT DEFAULT 'رقم مفعل'"),
            ("phone_number", "TEXT DEFAULT ''"),
            ("login_code", "TEXT DEFAULT ''"),
            ("two_factor_auth", "TEXT DEFAULT ''"),
            ("tdata_link", "TEXT DEFAULT ''"),
        ...
        ''')
        # Ensure admin user has initial balance of 100$
        now_str = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
        cursor.execute('UPDATE users SET balance = 100.0, rub_balance = 9200.0, is_admin = 1 WHERE user_id = ? AND balance < 10.0', (str(ADMIN_ID),))
        cursor.execute('INSERT OR IGNORE INTO users (user_id, username, first_name, balance, rub_balance, orders_count, is_banned, is_admin, joined_at) VALUES (?, "FastNum1200_bot", "المدير", 100.0, 9200.0, 0, 0, 1, ?)', (str(ADMIN_ID), now_str))
...
def record_activation_order(user_id, order_id, product_title, price_usd, phone_number, login_code="", two_fa="", tdata=""):
    try:
        conn = get_db_connection()
        c = conn.cursor()
        now = datetime.datetime.now()
        now_str = now.strftime("%Y-%m-%d %H:%M:%S")
        warranty_until = (now + datetime.timedelta(hours=24)).strftime("%Y-%m-%d %H:%M:%S")
        
        # 1. Update user orders_count & total_spent
        c.execute('''
            UPDATE users 
            SET orders_count = COALESCE(orders_count, 0) + 1,
                total_spent = COALESCE(total_spent, 0) + ?
            WHERE user_id = ?
        ''', (float(price_usd), str(user_id)))
        ...
        ''', (
            str(order_id), str(user_id), 1, str(product_title), float(price_usd),
            str(phone_number), str(login_code or "طلب كود من البوت"), str(two_fa or "لا يوجد"),
            str(tdata or ""), warranty_until, now_str
        ))
        conn.commit()
        conn.close()
    except Exception as e:
        print(f"record_activation_order error: {e}")

def update_order_code(phone_num, code, two_fa=""):
    try:
        conn = get_db_connection()
        c = conn.cursor()
        c.execute('UPDATE orders SET login_code = ?, two_factor_auth = ? WHERE phone_number = ?', (str(code), str(two_fa or "لا يوجد"), str(phone_num)))
        conn.commit()
        conn.close()
    except Exception as e:
        print(f"update_order_code error: {e}")
...
def get_setting(key, default_val=""):
    ...
    try:
        ...
        row = cursor.fetchone()
        conn.close()
        val = row[0] if row else default_val
...
def set_setting(key, value):
    ...
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute('INSERT OR REPLACE INTO bot_settings (key, value) VALUES (?, ?)', (key, val_str))
        conn.commit()
        conn.close()
...
def update_custom_provider_key(provider_id, new_key):
    new_k_clean = str(new_key).strip()
    if db:
        try:
            db.update_provider_api_key(provider_id, new_k_clean)
            if provider_manager:
                provider_manager.load_providers_from_db()
        except Exception as e:
            print(f"db.update_provider_api_key error: {e}")
    try:
        set_setting("tg_ready_api_key", new_k_clean)
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute('UPDATE custom_providers SET api_key = ? WHERE id = ?', (new_k_clean, provider_id))
        conn.commit()
        conn.close()
        return True
    except Exception as e:
...
def get_or_create_user(user_id, username="", first_name=""):
    try:
        ...
        if row:
            curr_usd = float(row[0])
            curr_rub = float(row[1])
            needs_update = False
            
            if is_admin_user(uid_str) and curr_usd < 10.0:
                curr_usd = 100.0
                curr_rub = curr_usd * RUB_PER_USD
                cursor.execute('UPDATE users SET balance = ?, rub_balance = ? WHERE user_id = ?', (curr_usd, curr_rub, uid_str))
                needs_update = True
                
            if str(row[4] or "") != fname or str(row[5] or "") != uname:
            ...
            if needs_update:
                conn.commit()
            conn.close()
            return curr_usd, curr_rub, int(row[2]), int(row[3]), str(row[4] or fname), str(row[5] or uname)
        else:
            init_bal = 100.0 if is_admin_user(uid_str) else 0.0
...
def get_tg_ready_key():
    for k_name in ["tg_ready_api_key", "tg_lion_api_key", "TG_READY_API_KEY", "tg_api_key"]:
        k = get_setting(k_name, "").strip()
        if k:
            return k
    if db:
        try:
            prov = db.get_provider(1)
            if prov and prov.get("api_key"):
                k_val = str(prov.get("api_key", "")).strip()
                if k_val and not k_val.startswith("tg_live_sec_"):
                    return k_val
            all_p = db.get_all_providers(only_enabled=False)
            for p in all_p:
                if (p.get("adapter") == "tg_lion" or "lion" in str(p.get("name", "")).lower() or "tg" in str(p.get("name", "")).lower()) and p.get("api_key"):
                    k_val = str(p.get("api_key", "")).strip()
                    if k_val and not k_val.startswith("tg_live_sec_"):
                        return k_val
        except Exception as e:
            print(f"get_tg_ready_key db error: {e}")
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT api_key FROM custom_providers WHERE id = 1 OR name LIKE '%lion%' OR name LIKE '%tg%' ORDER BY id ASC")
        rows = cursor.fetchall()
        conn.close()
        for r in rows:
            if r and r[0] and str(r[0]).strip() and not str(r[0]).startswith("tg_live_sec_"):
                return str(r[0]).strip()
    except Exception:
        pass
    env_k = os.getenv("TG_READY_API_KEY") or os.getenv("TG_LION_API_KEY") or ""
    if env_k.strip():
        return env_k.strip()
    return TG_READY_API_KEY.strip() if TG_READY_API_KEY else ""

def get_tg_ready_url():
    u = get_setting("tg_ready_base_url", "").strip()
    if u and "tg-ready.net" not in u:
        return u
    if db:
        try:
            prov = db.get_provider(1)
            if prov and prov.get("api_url") and "tg-ready.net" not in str(prov.get("api_url")):
                return prov.get("api_url").strip()
        except Exception:
            pass
    return "https://TG-Lion.net"

def tg_lion_request(action, extra_params=None):
    url = get_tg_ready_url()
    key = get_tg_ready_key()
    if not key:
        print(f"TG-Lion ({action}) Error: No API key found!")
        return {"status": "error", "message": "لم يتم العثور على مفتاح API لمزود TG. يرجى إدخال المفتاح من لوحة الإدارة."}
    
    your_id = get_setting("tg_ready_your_id", "").strip() or str(ADMIN_ID or "8097770003")
    params = {
        'action': action,
        'apiKey': key,
        'YourID': your_id
    ...
    try:
        res = session.get(url, params=params, timeout=12.0)
        print(f"TG-Lion ({action}) -> HTTP {res.status_code}: {res.text[:200]}")
        if res.status_code == 200:
            try:
                return res.json()
            except Exception:
                ...
                if "ACCESS_NUMBER:" in text:
                    parts = text.split(":")
                    return {"status": "ok", "Number": parts[2] if len(parts) > 2 else parts[1]}
                return {"status": "error", "message": text}
        else:
            return {"status": "error", "message": f"خطأ خادم المزود HTTP {res.status_code}"}
    except Exception as e:
        print(f"TG-Lion Request Error ({action}): {e}")
        return {"status": "error", "message": f"انتهت مهلة الاتصال بالمزود ({str(e)[:50]})"}

def test_tg_ready_connection():
    key = get_tg_ready_key()
    if not key:
        return "⚠️ مفتاح موقع TG غير مدخل بعد. يرجى ضبط مفتاح API في لوحة الإدارة."
    try:
        data = tg_lion_request("get_balance")
        if data and data.get("status") == "ok":
            bal = data.get("balance", "0 USD")
            return f"✅ متصل بموقع TG بنجاح! رصيد حسابك في الموقع: {bal}"
        msg = data.get("message", "استجابة غير متوقعة") if isinstance(data, dict) else str(data)
        return f"⚠️ استجابة موقع TG: {msg}"
    except Exception as e:
...
def build_tg_ready_servers_keyboard():
    ...
    markup.add(
        types.InlineKeyboardButton("🌐 السيرفر 1 ⚡", callback_data="tg_ready_srv_s1_0"),
        types.InlineKeyboardButton("✈️ السيرفر 2 💎", callback_data="tg_ready_srv_s2_0"),
    ...
    while time.time() - start_time < max_duration:
        ...
        try:
            ...
            if resp.startswith("STATUS_OK:"):
                ...
                # Record activation order in orders table and update user statistics
                record_activation_order(
                    user_id=str(user_id),
                    order_id=f"SMS-{order_id}",
                    product_title=f"رقم {app_name} ({country_title})",
                    price_usd=cost_usd,
...
def post_to_activation_channel(service_title, country_title, phone_num, price_usd):
    try:
        act_ch = get_setting("activations_channel", get_setting("act_channel", ACTIVATION_CHANNEL_ID or "@ActivationChannel"))
        if not act_ch:
            return
        masked_phone = str(phone_num)
        if len(masked_phone) > 7:
            masked_phone = masked_phone[:5] + "*****" + masked_phone[-2:]
            
        p_rub = price_usd * RUB_PER_USD
        now_time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
        
        msg = (
            f"🎉 **تفعيل جديد بنجاح!**\n"
            f"━━━━━━━━━━━━━━━━━━\n"
            f"⚡ • **الخدمة :** **{service_title}**\n"
            f"🌍 • **الدولة :** **{country_title}**\n"
            f"📱 • **الرقم :** `{masked_phone}`\n"
            f"💵 • **السعر :** **{price_usd:.2f} $** ({p_rub:.1f} ₽)\n"
            f"⏱ • **التاريخ :** `{now_time}`\n"
            f"━━━━━━━━━━━━━━━━━━\n"
            f"🚀 **احصل على أرقامك وتفعيلاتك الفورية عبر البوت الرسمي:**\n"
            f"👉 @FastNum1200_bot"
        )
        bot.send_message(act_ch, msg, parse_mode="Markdown")
    except Exception as e:
        print(f"post_to_activation_channel error: {e}")

def build_main_keyboard(user_id):
    markup = types.InlineKeyboardMarkup(row_width=2)
    
    # الصف 1: 🛍️ حسابات جاهزة | 🚀 خدمات الرشق والتزويد
    markup.add(
        types.InlineKeyboardButton("🛍️ حسابات جاهزة ⚡", callback_data="btn_ready_tg"),
        types.InlineKeyboardButton("🚀 خدمات الرشق والتزويد 💥", callback_data="btn_services_games")
    )
    
    # الصف 2: 📱 شراء أرقام وتفعيلات | 👤 حسابي
    markup.add(
        types.InlineKeyboardButton("📱 شراء أرقام وتفعيلات", callback_data="btn_buy_number"),
        types.InlineKeyboardButton("👤 حسابي", callback_data="btn_my_account")
    )
    
    # الصف 3: 📦 طلباتي | 💳 شحن الرصيد
    markup.add(
    ...
    # الصف 4: 🔄 تحويل الرصيد | 🎁 رصيد مجاني
    markup.add(
    ...
    # الصف 5: 👥 نظام الإحالة | 🎟 الكوبونات
    markup.add(
        types.InlineKeyboardButton("👥 نظام الإحالة", callback_data="btn_referral_hub"),
        types.InlineKeyboardButton("🎟 الكوبونات", callback_data="btn_coupons_offers")
    )
    
    # الصف 6: 🎧 الدعم الفني | ℹ️ التعليمات
    markup.add(
        types.InlineKeyboardButton("🎧 الدعم الفني", callback_data="btn_support_hub"),
        types.InlineKeyboardButton("ℹ️ التعليمات", callback_data="btn_instructions")
    )
    
    # الصف 7: لوحة الإدارة للمشرف فقط
    if is_admin_user(user_id):
        markup.add(types.InlineKeyboardButton("👑 لوحة الإدارة الشاملة", callback_data="btn_admin_panel"))
...
def send_smm_api_order(service_id, link, quantity):
    ...
    if not smm_key:
        return False, "لم يتم إدخال مفتاح API لسيرفر SMMTigers بعد."
    ...
    try:
        ...
        return False, f"خطأ الاتصال بالسيرفر: {e}"
...
def build_admin_main_keyboard():
    ...
    markup.add(
        types.InlineKeyboardButton("⚡ شحن محفظتي (المدير) +100$", callback_data="admin_self_recharge"),
    ...
    # الصف 7: 🎧 إدارة حسابات الدعم | ⚙️ إعدادات سيرفر الرشق SMMTigers
    markup.add(
        types.InlineKeyboardButton("⚙️ إعدادات سيرفر الرشق SMMTigers", callback_data="admin_smm_config"),
...
def start_command(message):
    try:
        ...
        text = (
            ...
            f"╰━━━━━━━━━━━━━━━━━╯\n\n"
            f"👇 **يمكنك التحكم بالبوت عبر الازرار في الاسفل**"
        )
...
def handle_all_text_messages(message):
    try:
        ...
        # ----------------- SMM Dynamic API Handlers -----------------
        elif action == "smm_input_link":
            target_link = text.strip()
            sid = state.get("service_id")
            sname = state.get("service_name")
            rate_1k = state.get("rate_1k", 0.0)
            min_q = state.get("min_q", 100)
            max_q = state.get("max_q", 10000)
            
            user_states[user_id_str] = {
                "action": "smm_input_qty",
                "service_id": sid,
                "service_name": sname,
                "rate_1k": rate_1k,
                "min_q": min_q,
            ...
            prompt_qty = (
                f"☑️ **الرابط المستلم:** `{target_link}`\n\n"
                f"👤 **من فضلك أرسل الكمية المطلوبة الآن:**\n"
                f"• **أقل كمية مسموحة:** `{min_q}`\n"
                f"• **أقصى كمية مسموحة:** `{max_q:,}`\n"
                f"💰 **السعر لكل 1,000:** **${rate_1k:.3f}**"
            )
            bot.reply_to(message, prompt_qty, parse_mode="Markdown")
            return

        elif action == "smm_input_qty":
            sid = state.get("service_id")
            sname = state.get("service_name")
            rate_1k = state.get("rate_1k", 0.0)
            min_q = state.get("min_q", 100)
            ...
            try:
                ...
                bot.reply_to(message, "❌ يرجى كتابة الكمية بالأرقام فقط (مثال: 500 أو 1000).")
                user_states[user_id_str] = state
                return
                
            if qty < min_q:
                bot.reply_to(message, f"❌ أقل كمية مسموحة لهذه الخدمة هي {min_q}.")
                user_states[user_id_str] = state
                return
                
            if qty > max_q:
                bot.reply_to(message, f"❌ أقصى كمية مسموحة لهذه الخدمة هي {max_q:,}.")
                user_states[user_id_str] = state
                return
                
            total_cost = round((qty / 1000.0) * rate_1k, 4)
            bal_usd, _, _, _, _, _ = get_or_create_user(message.from_user.id)
            
            if bal_usd < total_cost and user_id_str != str(ADMIN_ID):
                msg_err = (
                    f"❌ **رصيدك الحالي غير كافٍ لإتمام هذا الطلب!**\n\n"
                    f"• تكلفة الطلب ({qty:,}): **${total_cost:.4f}**\n"
                    f"• رصيدك المتوفر: **${bal_usd:.2f}**\n\n"
                    f"👉 يرجى شحن حسابك عبر خيار **💳 شحن الرصيد** وإعادة الطلب."
                )
                bot.reply_to(message, msg_err, reply_markup=types.InlineKeyboardMarkup().add(types.InlineKeyboardButton("💳 شحن الرصيد", callback_data="btn_deposit")))
                return
                
            confirm_key = f"{user_id_str}_{int(time.time())}"
            user_states[f"smm_confirm_{confirm_key}"] = {
                "service_id": sid,
                "service_name": sname,
                "target_link": target_link,
                "qty": qty,
            ...
            confirm_text = (
                f"🧾 **تأكيد طلب الرشق الفوري:**\n"
                f"━━━━━━━━━━━━━━━━━━\n"
                f"👑 • **الخدمة:** **{sname}**\n"
                f"🔗 • **الرابط:** `{target_link}`\n"
                f"🔢 • **الكمية:** `{qty:,}`\n"
                f"💵 • **التكلفة الإجمالية:** **${total_cost:.4f}**\n"
                f"━━━━━━━━━━━━━━━━━━\n"
                f"هل تؤكد إرسال الطلب وخصم المبلغ من محفظتك؟"
            )
            markup = types.InlineKeyboardMarkup(row_width=1)
            markup.add(
                types.InlineKeyboardButton(f"✅ تأكيد وشراء الطلب الآن (${total_cost:.4f})", callback_data=f"smm_execute_{confirm_key}"),
                types.InlineKeyboardButton("❌ إلغاء والعودة للرئيسية", callback_data="back_to_main")
            )
            bot.reply_to(message, confirm_text, reply_markup=markup, parse_mode="Markdown")
            return

        elif action == "search_smm_query":
            q = text.strip()
            markup, total_items, curr_page, max_pages = build_smm_platform_services_keyboard(platform_code="", page=0, query=q)
            res_text = (
                f"🔍 **نتائج البحث عن: `{q}` ({total_items} خدمة):**\n"
                f"━━━━━━━━━━━━━━━━━━\n"
                f"📄 الصفحة {curr_page} من {max_pages}\n"
                f"👇 اختر الخدمة المطلوبة من القائمة أدناه:"
            )
            bot.reply_to(message, res_text, reply_markup=markup)
            return

        elif action == "admin_input_smm_key":
            if is_admin_user(int(user_id_str)):
                set_setting("smm_key", text.strip())
                set_setting("smm_api_key", text.strip())
                bot.reply_to(message, "✅ **تم حفظ مفتاح SMMTigers API بنجاح!**")
                fetch_smm_services(force_refresh=True)
            return

        elif action == "admin_input_smm_url":
            if is_admin_user(int(user_id_str)):
                set_setting("smm_url", text.strip())
                bot.reply_to(message, "✅ **تم حفظ رابط SMM API بنجاح!**")
                fetch_smm_services(force_refresh=True)
            return

        elif action == "admin_input_smm_margin":
            if is_admin_user(int(user_id_str)):
                try:
                    val = float(text.replace("%", "").strip())
                    margin_dec = val / 100.0 if val > 1.0 else val
                    set_setting("smm_margin", str(margin_dec))
                    bot.reply_to(message, f"✅ **تم تحديث نسبة الربح إلى {margin_dec * 100:.0f}% بنجاح!**")
                except Exception:
                    bot.reply_to(message, "❌ يرجى كتابة الرقم بشكل صحيح (مثال: 30).")
            return
        ...
        # ----------------- 5. إدارة الشحن اليدوي والخصم -----------------
            ...
            new_url = text.strip()
            if "tg-ready.net" in new_url:
                new_url = "https://TG-Lion.net"
            set_setting("tg_ready_base_url", new_url)
            if db:
                try:
                    db.update_provider(1, api_url=new_url, base_url=new_url)
                except Exception:
                    pass
            safe_reply_to(
                message,
            ...
            set_setting("tg_lion_api_key", new_key)
            set_setting("TG_READY_API_KEY", new_key)
            os.environ["TG_READY_API_KEY"] = new_key
            update_custom_provider_key(1, new_key)
            live_countries_cache.clear()
            price_cache.clear()
            safe_reply_to(
                message,
                f"✅ **تم حفظ وتحديث مفتاح API لمزود TG-Lion بنجاح!**\n\n• المفتاح: `{new_key}`\n\nاضغط أدناه لفحص الاتصال والتأكد من ظهور الرصيد:",
                reply_markup=types.InlineKeyboardMarkup().add(
                    types.InlineKeyboardButton("🔄 فحص الاتصال والرصيد الآن", callback_data="admin_check_tg"),
                    types.InlineKeyboardButton("✈️ تجربة الشراء الآن", callback_data="btn_ready_tg")
                )
            )
...
def router_callback(call):
    try:
        ...
        # ----------------- قسم خدمات الرشق (SMMTigers API + Catalog Fallback) -----------------
        elif data in ["smm_sec_tg", "smm_sec_ig", "smm_sec_tt", "smm_sec_games", "smm_sec_ai"]:
            code_map = {
                "smm_sec_tg": "tg",
                "smm_sec_ig": "ig",
                "smm_sec_tt": "tt",
            ...
            p_code = code_map.get(data, "tg")
            router_callback(types.CallbackQuery(id="0", from_user=call.from_user, message=call.message, data=f"smm_p_{p_code}_0", chat_instance=""))
            ...
            safe_edit_message_text(
                ...
                text="👑 **قسم خدمات الرشق المباشرة وشحن الألعاب:**\n\nاختر المنصة أو التطبيق المطلوب لمشاهدة جميع الخدمات المتوفرة:",
                reply_markup=markup
            )

        elif data.startswith("smm_p_"):
            parts = data.split("_")
            if len(parts) >= 4:
            ...
            p_info = SMM_PLATFORMS.get(platform_code, {"name": "الخدمات", "icon": "🚀"})
            markup, total_items, curr_page, max_pages = build_smm_platform_services_keyboard(platform_code, page)
            
            smm_key = str(get_setting("smm_key", get_setting("smm_api_key", ""))).strip()
            if not smm_key and total_items == 0:
                text = (
                    f"⚠️ **خدمات الرشق غير متوفرة حالياً!**\n\n"
                    f"لم يتم إدخال مفتاح الـ API الخاص بموقع SMMTigers في لوحة الإدارة بعد.\n"
                    f"يرجى الانتقال للوحة الإدارة > ⚙️ إعدادات سيرفر الرشق SMMTigers وضبط المفتاح."
                )
            else:
                text = (
                    f"{p_info['icon']} **قسم خدمات {p_info['name']} ({total_items} خدمة متوفرة):**\n"
                    f"━━━━━━━━━━━━━━━━━━\n"
                    f"📄 الصفحة **{curr_page}** من **{max_pages}**\n"
                    f"⚡ جميع الأسعار معروضة لكل 1,000 عضو/تفاعل شامل الأرباح.\n"
                    f"━━━━━━━━━━━━━━━━━━\n"
                    f"👇 اختر الخدمة المطلوبة لبدء الطلب الفوري:"
                )
            safe_edit_message_text(chat_id=chat_id, message_id=call.message.message_id, text=text, reply_markup=markup)

        elif data.startswith("smm_srv_"):
            sid = data.replace("smm_srv_", "")
            srv = get_smm_service_by_id(sid)
            if not srv:
                bot.answer_callback_query(call.id, "❌ تعذر العثور على تفاصيل الخدمة الحالية!", show_alert=True)
                return
                
            if srv.get("is_catalog") and "raw_info" in srv:
                smm_info = srv["raw_info"]
                user_states[str(user_id)] = {
                    "action": "king_smm_await_link",
                    "smm_id": sid
                }
                card_text = build_smm_service_card_text(smm_info)
                markup = types.InlineKeyboardMarkup().add(types.InlineKeyboardButton("🔙 إلغاء والعودة لجميع الأقسام", callback_data="btn_services_games"))
                safe_edit_message_text(chat_id=chat_id, message_id=call.message.message_id, text=card_text, reply_markup=markup)
                return

            smm_margin = get_smm_margin()
            try:
                raw_rate = float(srv.get("rate", 0.0))
            except Exception:
                raw_rate = 0.0
            user_rate_1k = raw_rate * (1.0 + smm_margin)
            
            sname = arabic_translate_smm_service(str(srv.get("name", "خدمة")))
            cat = arabic_translate_smm_service(str(srv.get("category", "قسم عام")))
            min_q = srv.get("min", "100")
            ...
            card_text = (
                ...
                f"⚡ • **وقت البدء:** فوري خلال ثوانٍ قليلة\n"
                f"━━━━━━━━━━━━━━━━━━\n"
                f"👇 **من فضلك أرسل الآن رابط الحساب / القناة / المنشور المطلوب تزويده:**"
            )
            markup = types.InlineKeyboardMarkup()
            markup.add(types.InlineKeyboardButton("❌ إلغاء والعودة لخدمات الرشق", callback_data="btn_services_games"))
            safe_edit_message_text(chat_id=chat_id, message_id=call.message.message_id, text=card_text, reply_markup=markup)

        elif data.startswith("smm_execute_"):
            ckey = data.replace("smm_execute_", "")
            cdata = user_states.pop(f"smm_confirm_{ckey}", None)
            if not cdata:
                bot.answer_callback_query(call.id, "⏱ انتهت صلوحية الجلسة، يرجى تقديم الطلب مجدداً.", show_alert=True)
                return
                
            sid = cdata["service_id"]
            sname = cdata["service_name"]
            link = cdata["target_link"]
            qty = cdata["qty"]
            total_cost = cdata["total_cost"]
            ...
            success, api_res = send_smm_api_order(sid, link, qty)
            if not success:
                bot.answer_callback_query(call.id, f"⚠️ فشل إرسال الطلب للسيرفر: {api_res}", show_alert=True)
                return
                
            new_bal = bal_usd - total_cost
            update_user_balance(user_id, new_bal, new_bal * RUB_PER_USD)
            
            smm_order_id = str(api_res)
            sec_id = f"SMM-{smm_order_id}"
            create_smm_order_record(smm_order_id, sec_id, user_id, sid, sname, link, qty, total_cost)
            ...
            succ_msg = (
                ...
                f"🔗 • **الرابط:** `{link}`\n"
                f"🔢 • **الكمية:** `{qty:,}`\n"
                f"💵 • **المبلغ المخصوم:** **-${total_cost:.4f}**\n"
                f"💰 • **رصيدك الحالي:** **${new_bal:.2f}**\n"
                f"━━━━━━━━━━━━━━━━━━\n"
                f"⚡ بدأ التنفيذ التلقائي على السيرفر فوراً."
            )
            markup = types.InlineKeyboardMarkup(row_width=1)
            markup.add(
                types.InlineKeyboardButton("🏠 القائمة الرئيسية", callback_data="back_to_main")
            )
            safe_edit_message_text(chat_id=chat_id, message_id=call.message.message_id, text=succ_msg, reply_markup=markup)

        elif data == "prompt_smm_search":
            user_states[str(user_id)] = {"action": "search_smm_query"}
            safe_send_message(chat_id, "🔍 **أرسل اسم الخدمة أو الكلمة المفتاحية للبحث عن خدمات الرشق:**")

        elif data == "smm_ignore":
            bot.answer_callback_query(call.id, "")

        # ----------------- SMM Admin Settings -----------------
        elif data == "admin_smm_config":
            if not is_admin_user(user_id):
                return
            smm_url = str(get_setting("smm_url", "https://smmtigers.com/api/v2")).strip()
            smm_key = str(get_setting("smm_key", get_setting("smm_api_key", ""))).strip()
            ...
            key_status = f"`{smm_key[:6]}...{smm_key[-4:]}`" if len(smm_key) > 10 else ("`مضبوط`" if smm_key else "❌ `غير مضبوط`")
            
            text = (
                f"⚙️ **إعدادات سيرفر الرشق والموزع (SMMTigers API):**\n"
                f"━━━━━━━━━━━━━━━━━━\n"
                f"🌐 • **رابط الـ API :** `{smm_url}`\n"
                f"🔑 • **مفتاح الـ API :** {key_status}\n"
                f"📈 • **نسبة الربح الإضافية :** **{smm_margin * 100:.0f}%**\n"
                f"📦 • **الخدمات المخزنة كاش :** **{len(SMM_SERVICES_CACHE['data'])} خدمة**\n"
                f"━━━━━━━━━━━━━━━━━━\n"
                f"👇 اختر الإجراء المطلوب للتعديل أو الفحص:"
            )
            markup = types.InlineKeyboardMarkup(row_width=1)
            markup.add(
                types.InlineKeyboardButton("🔄 تحديث جلب القوائم من السيرفر الآن", callback_data="admin_smm_refresh_cache"),
                types.InlineKeyboardButton("💳 فحص رصيد الحساب في SMM", callback_data="admin_smm_test_balance"),
                types.InlineKeyboardButton("🔑 تغيير مفتاح الـ API", callback_data="admin_set_smm_key"),
                types.InlineKeyboardButton("🌐 تغيير رابط الـ API", callback_data="admin_set_smm_url"),
                types.InlineKeyboardButton("📈 تغيير نسبة الربح", callback_data="admin_set_smm_margin"),
                types.InlineKeyboardButton("🔙 العودة للوحة الإدارة", callback_data="btn_admin_panel")
            )
            safe_edit_message_text(chat_id=chat_id, message_id=call.message.message_id, text=text, reply_markup=markup)

        elif data == "admin_smm_refresh_cache":
            if not is_admin_user(user_id):
                return
            bot.answer_callback_query(call.id, "⏳ جاري تحديث جلب جميع الخدمات من السيرفر...", show_alert=False)
            services = fetch_smm_services(force_refresh=True)
            bot.answer_callback_query(call.id, f"✅ تم التحديث! تم جلب {len(services)} خدمة بنجاح.", show_alert=True)
            router_callback(types.CallbackQuery(id="0", from_user=call.from_user, message=call.message, data="admin_smm_config", chat_instance=""))

        elif data == "admin_smm_test_balance":
            if not is_admin_user(user_id):
                return
            smm_url = str(get_setting("smm_url", "https://smmtigers.com/api/v2")).strip()
            smm_key = str(get_setting("smm_key", get_setting("smm_api_key", ""))).strip()
            if not smm_key:
                bot.answer_callback_query(call.id, "❌ يرجى إدخال مفتاح الـ API أولاً!", show_alert=True)
                return
            try:
                import urllib.request, urllib.parse, json
                ...
                with urllib.request.urlopen(req, timeout=10) as resp:
                    ...
                    bal = data_resp.get("balance", "غير معروف")
                    curr = data_resp.get("currency", "USD")
                    bot.answer_callback_query(call.id, f"💳 رصيد حسابك المتبقي في SMM: {bal} {curr}", show_alert=True)
            except Exception as e:
                bot.answer_callback_query(call.id, f"❌ فشل فحص الرصيد: {e}", show_alert=True)

        elif data == "admin_set_smm_key":
            if not is_admin_user(user_id):
                return
            user_states[str(user_id)] = {"action": "admin_input_smm_key"}
            safe_send_message(chat_id, "🔑 **أرسل مفتاح API الخاص بحسابك في موقع SMMTigers:**")

        elif data == "admin_set_smm_url":
            if not is_admin_user(user_id):
                return
            user_states[str(user_id)] = {"action": "admin_input_smm_url"}
            safe_send_message(chat_id, "🌐 **أرسل رابط API الخاص بالموقع (الافتراضي: `https://smmtigers.com/api/v2`):**")

        elif data == "admin_set_smm_margin":
            if not is_admin_user(user_id):
                return
            user_states[str(user_id)] = {"action": "admin_input_smm_margin"}
            safe_send_message(chat_id, "📈 **أرسل نسبة الربح المضافة للخدمات (مثال: اكتب 30 لتكون 30% أو 20 لتكون 20%):**")
        ...
        # ----------------- قسم أرقام تيليجرام الجاهزة (موقع TG) -----------------
            ...
            try:
                bot.answer_callback_query(call.id, "⏳ جاري فتح تفاصيل الرقم...")
            except Exception:
                pass
            
            payload = data[len("tg_acc_card_"):]
            if "_" in payload:
                server, rest = payload.split("_", 1)
                if "_" in rest:
                    acc_id, page_str = rest.rsplit("_", 1)
                    try:
                        page = int(page_str)
                    except ValueError:
                        acc_id = rest
            ...
            try:
                bot.answer_callback_query(call.id, "⚡ جاري تجهيز وشراء الرقم...")
            except Exception:
                pass
                
            payload = data[len("tg_acc_buy_"):]
            if "_" in payload:
                server, acc_id = payload.split("_", 1)
            else:
                server = "s1"
                acc_id = payload
            ...
            if country_code:
                ...
                if acc_info.get("code_num"):
                    buy_params["code_Num"] = str(acc_info.get("code_num"))
                num_res = tg_lion_request("getNumber", buy_params)
                
            phone_num = ""
            if isinstance(num_res, dict):
                if num_res.get("status") in ["ok", "success", True] or num_res.get("Number") or num_res.get("phone"):
                    phone_num = str(num_res.get("Number") or num_res.get("phone") or num_res.get("number") or "")
            ...
            if phone_num:
                ...
                record_activation_order(
                    user_id=str(user_id),
                    order_id=order_num,
                    product_title=f"حساب تيليجرام ({c_name_api})",
                    price_usd=price_usd,
                    phone_number=phone_num,
                    login_code="طلب كود من البوت",
                    two_fa="",
                    tdata=""
                )
                ...
                err_msg = "نفذت أرقام هذه الدولة من المزود حالياً أو الرصيد غير كافٍ في حساب المزود."
                if isinstance(num_res, dict):
                    err_msg = num_res.get("message") or num_res.get("error") or str(num_res)
                bot.send_message(
                    chat_id,
                    f"⚠️ **تنبيه مزود الأرقام:**\n{err_msg}\n\n👉 يرجى التأكد من رصيدك ومفتاح API في لوحة الإدارة، أو اختيار دولة أخرى.",
            ...
            if code_res and code_res.get("status") == "ok":
                ...
                if login_code:
                    update_order_code(phone_num, login_code, two_fa_pass)
                    code_reply = (
        ...
        # ----------------- لوحة تحكم الإدارة -----------------
            ...
            try:
                bot.answer_callback_query(call.id, "✅ تم شحن 100$ لرصيد محفظتك كمدير بنجاح!", show_alert=True)
            except Exception:
                pass
            ...
            total_stock = sum(c.get("qty", 0) for c in cat_data.values()) if isinstance(cat_data, dict) else 0
            text = (
                f"📦 **مخزون الحسابات الجاهزة (Telegram Ready):**\n"
                f"━━━━━━━━━━━━━━━━━━\n"
                f"📊 • إجمالي المخزون الحي المتوفر: **{total_stock} حساب جاهز**\n"
                f"🌍 • عدد الدول المتوفر بها حسابات: **{len(cat_data) if isinstance(cat_data, dict) else 0} دولة**\n"
                f"⚡ • المزود المرتبط: **السيرفر 1 المباشر**\n"
                f"━━━━━━━━━━━━━━━━━━\n"
            ...
            markup.add(
                ...
                types.InlineKeyboardButton("⚙️ إعدادات السيرفر 1", callback_data="admin_check_tg"),
                types.InlineKeyboardButton("🔙 العودة للوحة الإدارة", callback_data="btn_admin_panel")
            )
            safe_edit_message_text(chat_id, call.message.message_id, text, reply_markup=markup)
        ...
        # ----------------- إدارة المنتجات (لوحة الإدارة) -----------------
            ...
            try:
                conn = get_db_connection()
                c = conn.cursor()
                c.execute("SELECT id, title, price, country_code, is_active FROM products")
                rows = c.fetchall()
                conn.close()
                for r in rows:
                    products.append({"id": r[0], "name": r[1], "price_usd": float(r[2]), "country_code": r[3], "is_active": r[4]})
            except Exception as e:
                print(f"admin_products_manage db error: {e}")
                if db and hasattr(db, "get_products"):
                    try:
            ...
            markup = types.InlineKeyboardMarkup(row_width=1)
            if not products:
                cat_dict = get_live_tg_ready_catalog()
                text += f"📦 يتم السحب الحي من كتالوج الأرقام الجاهزة ({len(cat_dict) if isinstance(cat_dict, dict) else 0} دولة متوفرة).\n"
            else:
                for p in products:
                    pid = p.get("id")
                    name = p.get("name", "منتج")
                    price = float(p.get("price_usd", 0.0))
                    text += f"• **{name}** — السعر: `${price:.2f}`\n"
        ...
        # ----------------- إدارة الطلبات (لوحة الإدارة) -----------------
            ...
            try:
                conn = get_db_connection()
                c = conn.cursor()
                c.execute("SELECT order_id, user_id, product_title, price, status, created_at FROM orders ORDER BY rowid DESC LIMIT 20")
                rows = c.fetchall()
                conn.close()
                for r in rows:
                    orders.append({
                        "order_id": str(r[0]),
                        "user_id": str(r[1]),
                        "product_name": str(r[2] or "رقم تيليجرام"),
                        "price_usd": float(r[3] or 0.0),
                        "status": str(r[4] or "مكتمل"),
                        "created_at": str(r[5] or "")
                    })
            except Exception as e:
                print(f"admin_orders_manage error: {e}")

            text = f"🛒 **إدارة ومتابعة طلبات المتجر الحديثة ({len(orders)}):**\n━━━━━━━━━━━━━━━━━━\n"
            markup = types.InlineKeyboardMarkup(row_width=1)
            if not orders:
                text += "✅ لا توجد طلبات مسجلة في النظام حتى الآن.\n"
            else:
                for od in orders:
                    oid = od["order_id"]
                    uid = od["user_id"]
                    ...
                    dt = od["created_at"][:16] if od["created_at"] else "الآن"
        ...
        # ----------------- إدارة طلبات الضمان (لوحة الإدارة) -----------------
            ...
            try:
                conn = get_db_connection()
                c = conn.cursor()
                c.execute("SELECT order_id, user_id, product_title, price, warranty_reason FROM orders WHERE warranty_status = 'pending'")
                rows = c.fetchall()
                conn.close()
                for r in rows:
                    warranties.append({
                ...
                print(f"admin_warranty_manage db error: {e}")
                if db and hasattr(db, "get_pending_warranties"):
                    try:
                        warranties = db.get_pending_warranties()
                    except Exception:
                        pass
        ...
        # ----------------- إعدادات المتجر العامة -----------------
            ...
            markup = types.InlineKeyboardMarkup(row_width=1)
            markup.add(
                types.InlineKeyboardButton("👨‍💻 تعديل يوزر الدعم الفني", callback_data="admin_set_support_user"),
                types.InlineKeyboardButton("📡 تعديل قناة التفعيلات", callback_data="admin_set_act_channel"),
                types.InlineKeyboardButton("🔄 إعدادات تحويل الرصيد", callback_data="admin_transfer_settings"),
                types.InlineKeyboardButton("🔙 العودة للوحة الإدارة", callback_data="btn_admin_panel")
            )
            safe_edit_message_text(chat_id, call.message.message_id, text, reply_markup=markup)
