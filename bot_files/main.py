...
def fivesim_buy_number(country_name, service_name):
    country_clean = str(country_name).lower().strip().replace(" ", "").replace("_", "").replace("-", "")
    ...
    last_error = "الأرقام لهذه الدولة غير متوفرة حالياً في 5SIM (نفذت الكمية المؤقتة)."
    ...
    for op in operators_to_try:
        try:
            ...
            if "not enough user balance" in lower_resp or "no balance" in lower_resp:
                return False, "رصيد السيرفر غير كافٍ لدى مزود 5SIM. يرجى إبلاغ الإدارة."
                ...
                last_error = "الأرقام لهذه الدولة غير متوفرة حالياً في 5SIM (نفذت الكمية المؤقتة). يرجى تجربة السيرفر 1 (Hero SMS) أو دولة أخرى."
                continue
            ...
            try:
                ...
                if "no free phones" in lower_resp:
                    continue
                last_error = f"تنبيه المزود (5SIM): {text_resp[:120]}"
                continue
            ...
            last_error = f"خطأ اتصال بـ 5SIM: {str(e)}"
            continue
...
def build_servers_keyboard(app_code):
    ...
    for cp in custom_provs:
        ...
        if cactive:
            markup.add(types.InlineKeyboardButton(f"⚡ سيرفر ({cname})", callback_data=f"page_{app_code}_custom_{cid}_0"))
