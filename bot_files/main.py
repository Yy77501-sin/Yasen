...
from catalog import MAIN_APPS, SERVICES, SERVERS, SMM_SECTIONS, SMM_SERVICES, PAYMENT_METHODS
...
DEFAULT_DB = {
    ...
    "settings": {
        ...
        "maintenance_msg": "🛠️ نعتذر منك! البوت في وضع الصيانة والتحديث حالياً بأمر الإدارة. سنعود للعمل قريباً جداً!",
        "smm_margin": 0.30,
        "number_margin": 0.25,
        "referral_reward": 0.10,
        "forced_channel": "",
        "forced_channel_enabled": False,
        ...
        "disabled_apps": [],
        "payment_methods": json.loads(json.dumps(PAYMENT_METHODS)),
    },
    "stats": {
...
def load_db():
    with DB_LOCK:
        ...
        # Merge settings subkeys
        if "settings" in data and isinstance(data["settings"], dict):
            for skey, sval in DEFAULT_DB["settings"].items():
                if skey not in data["settings"]:
                    data["settings"][skey] = json.loads(json.dumps(sval))
                    changed = True
...
def is_banned(user_id):
    try:
        uid = int(user_id)
        banned = DB.get("settings", {}).get("banned_users", [])
        return uid in banned or str(uid) in banned
    except Exception:
        return False
...
def is_admin(user_id):
    try:
        uid = int(user_id)
        if ADMIN_ID != 0 and uid == int(ADMIN_ID):
            return True
        admin_list = DB.get("settings", {}).get("admin_ids", [])
        if uid in admin_list or str(uid) in admin_list:
            return True
        # If no admin configured anywhere, allow access so creator/tester is never locked out
        if (ADMIN_ID == 0 or not ADMIN_ID) and not admin_list:
            return True
        return False
    except Exception:
        return False
...
def set_admin_command(message):
    ...
    admin_list = DB["settings"].setdefault("admin_ids", [])
    if uid not in admin_list:
        admin_list.append(uid)
        db_update()
        bot.reply_to(
            message,
            f"👑 <b>تم تعيينك بنجاح مديراً رئيسياً للبوت!</b>\n"
            f"🆔 ID: <code>{uid}</code>\n\n"
            "يمكنك الآن فتح لوحة الإدارة الكبرى عبر الأمر /admin أو زر الإدارة في القائمة الرئيسية.",
        )
    else:
        bot.reply_to(
            message,
            f"👑 <b>أنت مسجل بالفعل كمدير رئيسي في النظام!</b>\n"
            f"🆔 ID: <code>{uid}</code>\n"
            "استخدم /admin للدخول للوحة الإدارة الكبرى.",
        )


@bot.message_handler(commands=["admin", "panel", "control"])
def admin_command(message):
    uid = message.from_user.id
    if not is_admin(uid):
        bot.reply_to(
            message,
            "⛔ <b>ليس لديك صلاحية الدخول للوحة الإدارة.</b>\n\n"
            "إذا كنت مالك البوت أو المطور، أرسل <code>/setadmin</code> لتسجيل حسابك فوراً.",
        )
        return

    bot.send_message(
        message.chat.id,
        "👑 <b>لوحة الإدارة الكبرى - YasNum Grand Master Panel</b>\n\n"
        "أهلاً بك يا مدير النظام. يمكنك التحكم بكل شيء في البوت من الأقسام التالية:",
...
def start_command(message):
    try:
        ...
        if is_banned(uid):
            bot.send_message(
                message.chat.id,
                "⛔ <b>تم حظر حسابك من استخدام هذا البوت من قِبل إدارة النظام.</b>",
            )
            return

        # Maintenance mode check
        if DB.get("settings", {}).get("maintenance_mode", False) and not is_admin(uid):
            m_msg = DB.get("settings", {}).get("maintenance_msg", "🛠️ البوت في وضع الصيانة والتحديث حالياً.")
            bot.send_message(
                message.chat.id,
...
# =========================================================
# 👑 GRAND MASTER ADMIN PANEL (لوحة الإدارة الكبرى)
# =========================================================

def admin_menu():
    markup = types.InlineKeyboardMarkup(row_width=2)

    # Row 1: المواقع والمخازن
    markup.row(
        types.InlineKeyboardButton("🌐 المواقع ومزودي الخدمة", callback_data="admin_servers_manage"),
        types.InlineKeyboardButton("🏬 المخازن والتطبيقات", callback_data="admin_inventory_manage"),
    )

    # Row 2: طرق الشحن والمستخدمين
    markup.row(
        types.InlineKeyboardButton("💳 طرق الشحن والمحافظ", callback_data="admin_payments_manage"),
        types.InlineKeyboardButton("👥 المستخدمون والتحكم", callback_data="admin_users_manage"),
    )

    # Row 3: الأرباح والأسعار والطلبات
    markup.row(
        types.InlineKeyboardButton("💰 الأسعار وهامش الربح", callback_data="admin_pricing_manage"),
        types.InlineKeyboardButton("📦 مراقبة وسجل الطلبات", callback_data="admin_orders"),
    )

    # Row 4: الصيانة والاشتراك الإجباري والإذاعة
    markup.row(
        types.InlineKeyboardButton("🛡️ الصيانة وقناة الاشتراك", callback_data="admin_maintenance_manage"),
        types.InlineKeyboardButton("📢 إذاعة وبث جماعي", callback_data="admin_broadcast"),
    )

    # Row 5: الإحصائيات وفحص المزودين
    markup.row(
        types.InlineKeyboardButton("📊 التقارير المالية والربح", callback_data="admin_stats"),
        types.InlineKeyboardButton("🔄 فحص مباشر لمواقع التزويد", callback_data="admin_check_providers"),
    )

    # Row 6: الإعدادات والعودة
    markup.row(
...
# ---------------------------------------------------------
# 1. 🌐 إدارة المواقع ومزودي الخدمة (Providers & Servers)
# ---------------------------------------------------------

@bot.callback_query_handler(func=lambda call: call.data == "admin_servers_manage")
def cb_admin_servers_manage(call):
    if not is_admin(call.from_user.id):
        return

    active_srv = DB["settings"].get("active_servers", {})
    s1_on = active_srv.get("s1", True)
    s2_on = active_srv.get("s2", True)
    s3_on = active_srv.get("s3", True)
    s4_on = active_srv.get("s4", True)

    text = (
        "🌐 <b>لوحة إدارة المواقع ومزودي الخدمة المباشرين</b>\n\n"
        "تحكم بحالة السيرفرات ومفاتيح الربط والاتصال بمزودي الأرقام والرشق:\n\n"
        f"• <b>السيرفر 1 (5SIM.net):</b> {'🟢 مفعّل وشغال' if s1_on else '🔴 معطّل ومخفي'}\n"
        f"• <b>السيرفر 2 (Grizzly SMS):</b> {'🟢 مفعّل وشغال' if s2_on else '🔴 معطّل ومخفي'}\n"
        f"• <b>السيرفر 3 (Hero SMS):</b> {'🟢 مفعّل وشغال' if s3_on else '🔴 معطّل ومخفي'}\n"
        f"• <b>السيرفر 4 (Plus SMS VIP):</b> {'🟢 مفعّل وشغال' if s4_on else '🔴 معطّل ومخفي'}\n"
        "• <b>مزود الرشق (Plus SMM):</b> 🟢 مربوط بنظام الملك + 30% هامش\n\n"
        "💡 <i>اضغط على أي زر لتبديل حالة السيرفر فوراً (تفعيل / تعطيل):</i>"
    )

    markup = types.InlineKeyboardMarkup(row_width=2)
    markup.row(
        types.InlineKeyboardButton(f"{'🔴 إيقاف' if s1_on else '🟢 تفعيل'} 5SIM", callback_data="admin_toggle_srv:s1"),
        types.InlineKeyboardButton(f"{'🔴 إيقاف' if s2_on else '🟢 تفعيل'} Grizzly", callback_data="admin_toggle_srv:s2"),
    )
    markup.row(
        types.InlineKeyboardButton(f"{'🔴 إيقاف' if s3_on else '🟢 تفعيل'} Hero SMS", callback_data="admin_toggle_srv:s3"),
        types.InlineKeyboardButton(f"{'🔴 إيقاف' if s4_on else '🟢 تفعيل'} Plus SMS", callback_data="admin_toggle_srv:s4"),
    )
    markup.row(
        types.InlineKeyboardButton("🔄 فحص سرعة واستجابة المزودين", callback_data="admin_check_providers"),
    )
    markup.row(
...
@bot.callback_query_handler(func=lambda call: call.data.startswith("admin_toggle_srv:"))
def cb_admin_toggle_srv(call):
    if not is_admin(call.from_user.id):
        return

    srv_id = call.data.split(":", 1)[1]
    active_srv = DB["settings"].setdefault("active_servers", {"s1": True, "s2": True, "s3": True, "s4": True})
    current = active_srv.get(srv_id, True)
    active_srv[srv_id] = not current
    db_update()

    status_str = "تفعيل 🟢" if not current else "تعطيل 🔴"
    bot.answer_callback_query(call.id, f"تم {status_str} السيرفر بنجاح!")
    cb_admin_servers_manage(call)


# ---------------------------------------------------------
# 2. 🏬 إدارة المخازن والتطبيقات (Inventory & Supported Apps)
# ---------------------------------------------------------

@bot.callback_query_handler(func=lambda call: call.data == "admin_inventory_manage")
def cb_admin_inventory_manage(call):
    if not is_admin(call.from_user.id):
        return

    disabled = DB["settings"].get("disabled_apps", [])
    total_apps = len(SERVICES)
    active_count = total_apps - len(disabled)

    text = (
        "🏬 <b>إدارة مخزن التطبيقات والخدمات المدعومة</b>\n\n"
        f"📊 إجمالي التطبيقات المعتمدة: <b>{total_apps}</b>\n"
        f"🟢 التطبيقات النشطة في البوت: <b>{active_count}</b>\n"
        f"🔴 التطبيقات المعطلة مؤقتاً: <b>{len(disabled)}</b>\n\n"
        "يمكنك إيقاف أو تفعيل أي تطبيق بلمسة زر ليختفي أو يظهر للمستخدمين فوراً:"
    )

    markup = types.InlineKeyboardMarkup(row_width=2)
    row = []
    for code, info in SERVICES.items():
        is_off = code in disabled
        btn_text = f"{'🔴' if is_off else '🟢'} {info['short']}"
        row.append(types.InlineKeyboardButton(btn_text, callback_data=f"admin_toggle_app:{code}"))
        if len(row) == 2:
            markup.row(*row)
            row = []
    if row:
    ...
    markup.row(types.InlineKeyboardButton("🔙 لوحة الإدارة", callback_data="admin_panel"))

    bot.answer_callback_query(call.id)
    bot.edit_message_text(text, call.message.chat.id, call.message.message_id, reply_markup=markup)


@bot.callback_query_handler(func=lambda call: call.data.startswith("admin_toggle_app:"))
def cb_admin_toggle_app(call):
    if not is_admin(call.from_user.id):
        return

    code = call.data.split(":", 1)[1]
    disabled = DB["settings"].setdefault("disabled_apps", [])

    if code in disabled:
        disabled.remove(code)
        action_str = "تفعيل التطبيق في المخزن 🟢"
    else:
        disabled.append(code)
        action_str = "إيقاف التطبيق مؤقتاً 🔴"

    db_update()
    bot.answer_callback_query(call.id, f"تم {action_str}!")
    cb_admin_inventory_manage(call)


# ---------------------------------------------------------
# 3. 💳 إدارة طرق الشحن والمحافظ (Payment Methods)
# ---------------------------------------------------------

@bot.callback_query_handler(func=lambda call: call.data == "admin_payments_manage")
def cb_admin_payments_manage(call):
    if not is_admin(call.from_user.id):
        return

    methods = DB["settings"].get("payment_methods", PAYMENT_METHODS)

    lines = []
    for pid, m in methods.items():
        status = "🟢 مفعّل" if m.get("enabled", True) else "🔴 معطّل"
        lines.append(f"• <b>{m['name']}</b> ({status})\n  └ العنوان/الحساب: <code>{safe_html(m.get('address', 'غير محدد'))}</code>")

    text = (
        "💳 <b>إدارة طرق الشحن ومحافظ الإيداع الكبرى</b>\n\n"
        "تحكم بجميع بوابات الدفع، أرقام الحسابات، وعناوين المحافظ المتاحة للمستخدمين:\n\n"
        + "\n".join(lines)
        + "\n\n💡 <i>اختر وسيلة لتعديل رقمها/عنوانها أو تفعيلها/تعطيلها:</i>"
    )

    markup = types.InlineKeyboardMarkup(row_width=2)
    for pid, m in methods.items():
        state_icon = "🔴 إيقاف" if m.get("enabled", True) else "🟢 تشغيل"
        markup.row(
            types.InlineKeyboardButton(f"✏️ تعديل {m['name']}", callback_data=f"admin_edit_pay_addr:{pid}"),
            types.InlineKeyboardButton(f"{state_icon}", callback_data=f"admin_toggle_pay:{pid}"),
        )

    markup.row(
        types.InlineKeyboardButton("🧾 طلبات الشحن والإيداعات المعلقة", callback_data="admin_recharges"),
    )
    markup.row(
        types.InlineKeyboardButton("🔙 لوحة الإدارة", callback_data="admin_panel"),
    )
...
@bot.callback_query_handler(func=lambda call: call.data.startswith("admin_toggle_pay:"))
def cb_admin_toggle_pay(call):
    if not is_admin(call.from_user.id):
        return

    pid = call.data.split(":", 1)[1]
    methods = DB["settings"].setdefault("payment_methods", json.loads(json.dumps(PAYMENT_METHODS)))
    if pid in methods:
        methods[pid]["enabled"] = not methods[pid].get("enabled", True)
        db_update()
        bot.answer_callback_query(call.id, f"تم تبديل حالة وسيلة {methods[pid]['name']}!")

    cb_admin_payments_manage(call)


@bot.callback_query_handler(func=lambda call: call.data.startswith("admin_edit_pay_addr:"))
def cb_admin_edit_pay_addr(call):
    if not is_admin(call.from_user.id):
        return
    ...
    set_state(call.from_user.id, "admin_wait_pay_address", pid=pid)

    bot.answer_callback_query(call.id)
    bot.edit_message_text(
        f"✏️ <b>تعديل حساب/محفظة: {safe_html(m.get('name', pid))}</b>\n\n"
        f"العنوان الحالي: <code>{safe_html(m.get('address', 'غير محدد'))}</code>\n\n"
        "أرسل الآن العنوان أو رقم الحساب الجديد في رسالة نصية ليتم اعتماده فوراً:",
        call.message.chat.id,
        call.message.message_id,
        reply_markup=back_to("admin_payments_manage"),
    )


# ---------------------------------------------------------
# 4. 👥 إدارة المستخدمين الشاملة (Full User Management)
# ---------------------------------------------------------

@bot.callback_query_handler(func=lambda call: call.data == "admin_users_manage")
def cb_admin_users_manage(call):
    if not is_admin(call.from_user.id):
        return

    users = DB["users"]
    banned = DB["settings"].get("banned_users", [])
    total_balance = sum(float(u.get("balance", 0) or 0) for u in users.values())

    text = (
        "👥 <b>إدارة المستخدمين الشاملة</b>\n\n"
        f"👤 إجمالي المستخدمين المسجلين: <b>{len(users)}</b>\n"
        f"🚫 المستخدمون المحظورون: <b>{len(banned)}</b>\n"
        f"💰 مجموع أرصدة المستخدمين: <b>${total_balance:.2f}</b>\n\n"
        "يمكنك البحث عن أي مستخدم عبر Telegram ID لشحن رصيده، خصم رصيد، حظره، أو مراسلته:"
    )

    markup = types.InlineKeyboardMarkup(row_width=2)
    markup.row(
        types.InlineKeyboardButton("🔍 بحث عن مستخدم بالآيدي", callback_data="admin_find_user"),
        types.InlineKeyboardButton("💰 شحن / خصم رصيد", callback_data="admin_balance"),
    )
    markup.row(
        types.InlineKeyboardButton("📋 آخر 10 مستخدمين انضموا", callback_data="admin_recent_users"),
    )
    markup.row(
        types.InlineKeyboardButton("🔙 لوحة الإدارة", callback_data="admin_panel"),
    )
...
def cb_admin_recent_users(call):
    if not is_admin(call.from_user.id):
        return
    ...
    for u in reversed(users):
        status = "🚫 محظور" if is_banned(u.get("id")) else "🟢 نشط"
        lines.append(
            f"• <code>{u.get('id')}</code> | {safe_html(u.get('first_name', 'مستخدم'))} | ${float(u.get('balance', 0)):.2f} | {status}"
        )

    text = "📋 <b>آخر المستخدمين المسجلين في البوت:</b>\n\n" + ("\n".join(lines) if lines else "لا يوجد مستخدمون بعد.")

    bot.answer_callback_query(call.id)
    bot.edit_message_text(text, call.message.chat.id, call.message.message_id, reply_markup=back_to("admin_users_manage"))
...
def cb_admin_ban_user(call):
    ...
    banned = DB["settings"].setdefault("banned_users", [])
    ...
    if uid in banned:
        ...
        banned.append(uid)
        bot.answer_callback_query(call.id, "🚫 تم حظر المستخدم نهائياً!")

    db_update()
    # Refresh user view
    target = get_user(uid)
    if target:
        show_admin_user_profile(call.message.chat.id, call.message.message_id, target)


def show_admin_user_profile(chat_id, message_id, target):
    uid = target["id"]
    banned = is_banned(uid)
    status_str = "🔴 محظور من البوت" if banned else "🟢 نشط وطبيعي"

    text = (
        "👤 <b>الملف الكامل للمستخدم في الإدارة</b>\n\n"
        f"🆔 الآيدي (Telegram ID): <code>{uid}</code>\n"
        f"👤 الاسم: <b>{safe_html(target.get('first_name'))}</b>\n"
        f"🔗 المعرف: @{safe_html(target.get('username')) if target.get('username') else 'بدون معرف'}\n"
        f"💰 الرصيد الحالي: <b>${float(target.get('balance', 0)):.2f}</b>\n"
        f"💱 العملة المفضلة: <b>{target.get('currency', 'USD')}</b>\n"
        f"📦 المشتريات الناجحة: <b>{target.get('successful_orders', 0)}</b>\n"
        f"👥 عدد الإحالات: <b>{len(target.get('referrals', []))}</b>\n"
        f"⚡ الحالة الحالية: <b>{status_str}</b>"
    )

    markup = types.InlineKeyboardMarkup(row_width=2)
    markup.row(
        types.InlineKeyboardButton("➕ شحن رصيد", callback_data=f"admin_quick_bal:{uid}:add"),
        types.InlineKeyboardButton("➖ خصم رصيد", callback_data=f"admin_quick_bal:{uid}:sub"),
    )
    markup.row(
        types.InlineKeyboardButton("✉️ إرسال رسالة خاصة", callback_data=f"admin_user_msg:{uid}"),
        types.InlineKeyboardButton(f"{'✅ إلغاء الحظر' if banned else '🚫 حظر المستخدم'}", callback_data=f"admin_ban_user:{uid}"),
    )
    markup.row(
...
@bot.callback_query_handler(func=lambda call: call.data.startswith("admin_quick_bal:"))
def cb_admin_quick_bal(call):
    if not is_admin(call.from_user.id):
        return

    _, uid_str, op = call.data.split(":", 2)
    set_state(call.from_user.id, "admin_wait_balance_amount", target_id=int(uid_str), op=op)

    bot.answer_callback_query(call.id)
    op_ar = "إضافة إلى" if op == "add" else "خصم من"
    bot.edit_message_text(
        f"💰 <b>{op_ar} رصيد المستخدم <code>{uid_str}</code></b>\n\n"
        "أرسل المبلغ الذي تريد تنفيذه (مثال: <code>5</code> أو <code>10.5</code>):",
        call.message.chat.id,
        call.message.message_id,
        reply_markup=back_to("admin_users_manage"),
    )


@bot.callback_query_handler(func=lambda call: call.data.startswith("admin_user_msg:"))
def cb_admin_user_msg(call):
    if not is_admin(call.from_user.id):
        return

    uid_str = call.data.split(":", 1)[1]
    set_state(call.from_user.id, "admin_wait_user_dm", target_id=int(uid_str))

    bot.answer_callback_query(call.id)
    bot.edit_message_text(
        f"✉️ <b>إرسال رسالة خاصة للمستخدم <code>{uid_str}</code></b>\n\n"
        "أرسل نص الرسالة التي تريد أن تصله باسم إدارة البوت:",
        call.message.chat.id,
        call.message.message_id,
        reply_markup=back_to("admin_users_manage"),
    )
...
# ---------------------------------------------------------
# 5. 💰 الأسعار وهامش الربح (Pricing Engine & Margins)
# ---------------------------------------------------------

@bot.callback_query_handler(func=lambda call: call.data == "admin_pricing_manage")
def cb_admin_pricing_manage(call):
    if not is_admin(call.from_user.id):
        return

    smm_margin = float(DB["settings"].get("smm_margin", 0.30)) * 100
    num_margin = float(DB["settings"].get("number_margin", 0.25)) * 100
    ref_reward = float(DB["settings"].get("referral_reward", 0.10))

    text = (
        "💰 <b>إدارة الأسعار وهوامش الأرباح التلقائية</b>\n\n"
        f"📈 <b>هامش ربح خدمات الرشق (Plus SMM):</b> <code>+{smm_margin:.0f}%</code>\n"
        "  (يُضاف تلقائياً إلى سعر الجملة الخام من المزود، مثلاً 0.271$ تصبح 0.35$)\n\n"
        f"📱 <b>هامش ربح الأرقام الافتراضية:</b> <code>+{num_margin:.0f}%</code>\n"
        "  (يُضاف فوق أسعار مزودي الأرقام 5SIM, Grizzly, Hero)\n\n"
        f"🎁 <b>مكافأة الإحالة لكل دعوة:</b> <code>${ref_reward:.2f}</code>\n\n"
        "اختر ما تريد تعديله فوراً:"
    )

    markup = types.InlineKeyboardMarkup(row_width=1)
    markup.row(types.InlineKeyboardButton("✏️ تعديل نسبة ربح الرشق (%)", callback_data="admin_set_smm_margin"))
    markup.row(types.InlineKeyboardButton("✏️ تعديل نسبة ربح الأرقام (%)", callback_data="admin_set_num_margin"))
    markup.row(types.InlineKeyboardButton("✏️ تعديل قيمة مكافأة الإحالة ($)", callback_data="admin_set_ref_reward"))
    markup.row(types.InlineKeyboardButton("🔙 لوحة الإدارة", callback_data="admin_panel"))
...
@bot.callback_query_handler(func=lambda call: call.data == "admin_set_smm_margin")
def cb_admin_set_smm_margin(call):
    if not is_admin(call.from_user.id):
        return
    set_state(call.from_user.id, "admin_wait_smm_margin")
    bot.answer_callback_query(call.id)
    bot.edit_message_text(
        "✏️ <b>تعديل هامش ربح الرشق</b>\n\nأرسل النسبة المئوية الجديدة (مثال: <code>35</code> لـ 35% أو <code>50</code> لـ 50%):",
        call.message.chat.id,
        call.message.message_id,
        reply_markup=back_to("admin_pricing_manage"),
    )


@bot.callback_query_handler(func=lambda call: call.data == "admin_set_num_margin")
def cb_admin_set_num_margin(call):
    if not is_admin(call.from_user.id):
        return
    set_state(call.from_user.id, "admin_wait_num_margin")
    bot.answer_callback_query(call.id)
    bot.edit_message_text(
        "✏️ <b>تعديل هامش ربح الأرقام الافتراضية</b>\n\nأرسل النسبة المئوية الجديدة (مثال: <code>20</code> لـ 20% أو <code>30</code> لـ 30%):",
        call.message.chat.id,
        call.message.message_id,
        reply_markup=back_to("admin_pricing_manage"),
    )
...
def cb_admin_set_ref_reward(call):
    if not is_admin(call.from_user.id):
        return
    set_state(call.from_user.id, "admin_wait_ref_reward")
    bot.answer_callback_query(call.id)
    bot.edit_message_text(
        "✏️ <b>تعديل مكافأة الإحالة</b>\n\nأرسل المبلغ بالدولار لكل دعوة جديدة (مثال: <code>0.10</code> أو <code>0.25</code>):",
        call.message.chat.id,
        call.message.message_id,
        reply_markup=back_to("admin_pricing_manage"),
    )
...
# ---------------------------------------------------------
# 6. 🛡️ الصيانة والاشتراك الإجباري (Maintenance & Forced Channel)
# ---------------------------------------------------------

@bot.callback_query_handler(func=lambda call: call.data == "admin_maintenance_manage")
def cb_admin_maintenance_manage(call):
    if not is_admin(call.from_user.id):
        return

    is_maint = DB["settings"].get("maintenance_mode", False)
    forced_on = DB["settings"].get("forced_channel_enabled", False)
    channel = DB["settings"].get("forced_channel", "غير محدد")

    text = (
        "🛡️ <b>نظام الحماية والصيانة والاشتراك الإجباري</b>\n\n"
        f"🛠️ <b>وضع الصيانة:</b> {'🔴 مُفعّل (البوت مقفل للزوار)' if is_maint else '🟢 مُعطّل (البوت يعمل بشكل طبيعي)'}\n"
        f"📢 <b>الاشتراك الإجباري:</b> {'🟢 مُفعّل' if forced_on else '🔴 مُعطّل'}\n"
        f"🔗 <b>قناة الاشتراك المحددة:</b> <code>{safe_html(channel)}</code>\n\n"
        "💡 <i>عند تفعيل وضع الصيانة، لا يستطيع المستخدمون الشراء، بينما يمكنك كأدمن الدخول والتجربة بحرية كاملة.</i>"
    )

    markup = types.InlineKeyboardMarkup(row_width=1)
    markup.row(
        types.InlineKeyboardButton(f"{'🟢 إيقاف وضع الصيانة' if is_maint else '🔴 تفعيل وضع الصيانة فوراً'}", callback_data="admin_toggle_maintenance"),
    )
    markup.row(
        types.InlineKeyboardButton(f"{'🔴 إيقاف الاشتراك الإجباري' if forced_on else '🟢 تفعيل الاشتراك الإجباري'}", callback_data="admin_toggle_force_channel"),
    )
    markup.row(
        types.InlineKeyboardButton("✏️ تعيين معرف قناة الاشتراك الإجباري", callback_data="admin_set_channel"),
    )
    markup.row(
        types.InlineKeyboardButton("🔙 لوحة الإدارة", callback_data="admin_panel"),
    )
...
def cb_admin_toggle_maintenance(call):
    if not is_admin(call.from_user.id):
        return

    curr = DB["settings"].get("maintenance_mode", False)
    DB["settings"]["maintenance_mode"] = not curr
    db_update()

    bot.answer_callback_query(call.id, f"تم {'تفعيل 🔴' if not curr else 'إيقاف 🟢'} وضع الصيانة بنجاح!")
    cb_admin_maintenance_manage(call)


@bot.callback_query_handler(func=lambda call: call.data == "admin_toggle_force_channel")
def cb_admin_toggle_force_channel(call):
    if not is_admin(call.from_user.id):
        return

    curr = DB["settings"].get("forced_channel_enabled", False)
    DB["settings"]["forced_channel_enabled"] = not curr
    db_update()

    bot.answer_callback_query(call.id, f"تم {'تفعيل 🟢' if not curr else 'إيقاف 🔴'} الاشتراك الإجباري!")
    cb_admin_maintenance_manage(call)


@bot.callback_query_handler(func=lambda call: call.data == "admin_set_channel")
def cb_admin_set_channel(call):
    if not is_admin(call.from_user.id):
        return

    set_state(call.from_user.id, "admin_wait_channel_name")
    bot.answer_callback_query(call.id)
    bot.edit_message_text(
        "📢 <b>تعيين قناة الاشتراك الإجباري</b>\n\n"
        "أرسل معرف القناة أو رابطها (مثال: <code>@YasNumChannel</code>):\n"
        "⚠️ تأكد من رفع البوت كمشرف (Admin) في القناة ليتمكن من فحص اشتراك الأعضاء.",
        call.message.chat.id,
        call.message.message_id,
        reply_markup=back_to("admin_maintenance_manage"),
    )


# ---------------------------------------------------------
# 7. 🌐 فحص السيرفرات والمزودين (Providers Status)
# ---------------------------------------------------------
...
def cb_admin_check_providers(call):
    ...
    for prov in providers_to_test:
        ...
        key_status = "🔑 مفعل ومربوط" if key else "⚠️ مفتاح مدمج"
...
# ---------------------------------------------------------
# 8. 👑 أوامر الإدارة ولوحة التحكم السريعة
# ---------------------------------------------------------

@bot.callback_query_handler(func=lambda call: call.data == "admin_panel")
def cb_admin_panel(call):
    if not is_admin(call.from_user.id):
        bot.answer_callback_query(call.id, "⛔ ليس لديك صلاحية أدمن. أرسل /setadmin لتعيين نفسك مديراً.", show_alert=True)
        return

    bot.answer_callback_query(call.id)
    bot.edit_message_text(
        "👑 <b>لوحة الإدارة الكبرى - YasNum Grand Master Panel</b>\n\n"
        "أهلاً بك يا مدير النظام. تحكم بالبوت تحكماً شاملاً عبر الأقسام المنظمة التالية:",
        call.message.chat.id,
        call.message.message_id,
        reply_markup=admin_menu(),
    )
...
def cb_admin_users(call):
    return cb_admin_users_manage(call)
...
def cb_admin_find_user(call):
    ...
    bot.answer_callback_query(call.id)
    bot.edit_message_text(
        "🔍 <b>بحث عن مستخدم بالآيدي</b>\n\n"
        "أرسل Telegram ID للمستخدم الذي تريد البحث عنه وإدارته:",
        call.message.chat.id,
        call.message.message_id,
        reply_markup=back_to("admin_users_manage"),
    )
...
def cb_admin_orders(call):
    ...
    if not orders:
        body = "لا توجد طلبات مسجلة حتى الآن."
    else:
        lines = []
...
def cb_admin_recharges(call):
    ...
    if not records:
        body = "لا توجد طلبات شحن مسجلة."
    else:
        lines = []
        for r in reversed(records):
            status_icon = "⏳ معلق" if r.get("status") in ("pending", "submitted") else ("✅ مقبول" if r.get("status") == "approved" else "❌ مرفوض")
            lines.append(
...
def cb_admin_stats(call):
    ...
    total_balance = sum(float(u.get("balance", 0) or 0) for u in DB["users"].values())
    banned_count = len(DB["settings"].get("banned_users", []))
    total_recharged = sum(float(r.get("amount", 0) or 0) for r in DB["recharges"].values() if r.get("status") == "approved")

    text = (
        "📊 <b>التقارير المالية وإحصائيات الإدارة الشاملة</b>\n\n"
        f"👥 إجمالي المستخدمين: <b>{len(DB['users'])}</b> (محظور: {banned_count})\n"
        f"💰 مجموع أرصدة المستخدمين الحالية: <b>${total_balance:.2f}</b>\n"
        f"💵 إجمالي المبالغ المشحونة المقبولة: <b>${total_recharged:.2f}</b>\n"
        f"📦 إجمالي طلبات الشراء: <b>{len(DB['orders'])}</b>\n"
        f"✅ عمليات الشراء الناجحة: <b>{DB['stats'].get('successful_purchases', 0)}</b>\n"
        f"💳 إجمالي طلبات الشحن: <b>{len(DB['recharges'])}</b>\n"
        f"📈 هامش ربح الرشق: <b>+{float(DB['settings'].get('smm_margin', 0.30))*100:.0f}%</b>\n"
        f"📱 هامش ربح الأرقام: <b>+{float(DB['settings'].get('number_margin', 0.25))*100:.0f}%</b>"
    )

    bot.answer_callback_query(call.id)
    bot.edit_message_text(text, call.message.chat.id, call.message.message_id, reply_markup=back_to("admin_panel"))
...
def cb_admin_broadcast(call):
    ...
    bot.answer_callback_query(call.id)
    bot.edit_message_text(
        "📢 <b>إذاعة وبث رسالة جماعية</b>\n\n"
        "أرسل الرسالة التي تريد إرسالها لجميع المشتركين في البوت فوراً.\n\n"
        "⚠️ ستصل الرسالة لكل مستخدم في رسالة خاصة من البوت.",
        call.message.chat.id,
        call.message.message_id,
        reply_markup=back_to("admin_panel"),
    )
...
def cb_admin_settings(call):
    ...
    admin_ids = DB["settings"].get("admin_ids", [])
    admin_str = ", ".join(str(a) for a in admin_ids) if admin_ids else str(ADMIN_ID)

    text = (
        "⚙️ <b>إعدادات النظام العامة والمطور</b>\n\n"
        f"🤖 اسم البوت: <b>{safe_html(BOT_NAME)}</b>\n"
        f"👑 مدراء البوت المعتمدون: <code>{admin_str}</code>\n"
        f"💰 مكافأة الإحالة: <b>${float(DB['settings'].get('referral_reward', 0.10)):.2f}</b>\n"
        f"🎧 حساب الدعم الفني: <b>{safe_html(SUPPORT_USERNAME)}</b>\n"
        f"💱 العملة الأساسية: <b>{DB['settings'].get('currency', 'USD')}</b>\n"
        f"⚡ إصدار النظام: <b>Grand Master v2.5</b>"
    )
...
def text_state_handler(message):
    ...
    if state == "admin_wait_user_id":
        ...
        if not target:
            bot.reply_to(message, "❌ المستخدم غير موجود في قاعدة بيانات البوت.")
            return

        clear_state(message.from_user.id)
        show_admin_user_profile(message.chat.id, None, target)
        return
    ...
    if state == "admin_wait_balance_user":
        ...
        bot.send_message(
            message.chat.id,
            f"👤 المستخدم: <code>{text}</code> (رصيده الحالي: ${float(target.get('balance', 0)):.2f})\n\n"
            "أرسل المبلغ المراد:\n"
    ...
    if state == "admin_wait_balance_amount":
        ...
        bot.send_message(
            message.chat.id,
            f"✅ <b>تم {action_word} الرصيد بنجاح</b>\n\n"
        ...
        try:
            bot.send_message(
                target_id,
                f"💰 <b>تم تحديث رصيد محفظتك من قِبل الإدارة</b>\n\n"
                f"🔄 التغيير: <b>${amount:+.2f}</b>\n"
    ...
    # -----------------------------------------------------
    # Admin payment address update
    # -----------------------------------------------------

    if state == "admin_wait_pay_address":
        if not is_admin(message.from_user.id):
            clear_state(message.from_user.id)
            return
        ...
        if pid in methods:
            methods[pid]["address"] = text.strip()
            db_update()
            clear_state(message.from_user.id)
            bot.send_message(
                message.chat.id,
                f"✅ <b>تم تحديث عنوان وسيلة {methods[pid]['name']} بنجاح!</b>\n\n"
                f"العنوان الجديد المعتمد:\n<code>{safe_html(text.strip())}</code>",
                reply_markup=back_to("admin_payments_manage"),
            )
        else:
            clear_state(message.from_user.id)
            bot.send_message(message.chat.id, "❌ خطأ في تحديد وسيلة الدفع.", reply_markup=back_to("admin_payments_manage"))
        return
    ...
    if state == "admin_wait_user_dm":
        if not is_admin(message.from_user.id):
            clear_state(message.from_user.id)
            return

        target_id = int(state_data.get("target_id", 0))
        clear_state(message.from_user.id)

        try:
            bot.send_message(
                target_id,
                f"📩 <b>رسالة رسمية من إدارة البوت:</b>\n\n{safe_html(text)}"
            )
            bot.send_message(
                message.chat.id,
                f"✅ <b>تم إرسال الرسالة بنجاح إلى المستخدم <code>{target_id}</code>!</b>",
                reply_markup=back_to("admin_users_manage"),
            )
        except Exception as e:
            bot.send_message(
                message.chat.id,
                f"❌ تعذر إرسال الرسالة إلى المستخدم (قد يكون قام بحظر البوت):\n<code>{safe_html(str(e))}</code>",
                reply_markup=back_to("admin_users_manage"),
            )
        return

    # -----------------------------------------------------
    # Admin SMM profit margin
    # -----------------------------------------------------

    if state == "admin_wait_smm_margin":
        if not is_admin(message.from_user.id):
            clear_state(message.from_user.id)
            return
        ...
        try:
            ...
            DB["settings"]["smm_margin"] = round(val / 100.0, 4)
            db_update()
            clear_state(message.from_user.id)
            bot.send_message(
                message.chat.id,
                f"✅ <b>تم تعديل هامش ربح الرشق إلى {val:.0f}%</b> بنجاح!",
                reply_markup=back_to("admin_pricing_manage"),
            )
        except ValueError:
            bot.reply_to(message, "❌ يرجى إدخال رقم صحيح (مثال: 30)")
        return

    # -----------------------------------------------------
    # Admin Number profit margin
    # -----------------------------------------------------

    if state == "admin_wait_num_margin":
        if not is_admin(message.from_user.id):
            clear_state(message.from_user.id)
            return
        ...
        try:
            val = float(text.replace("%", "").strip())
            DB["settings"]["number_margin"] = round(val / 100.0, 4)
            db_update()
            clear_state(message.from_user.id)
            bot.send_message(
                message.chat.id,
                f"✅ <b>تم تعديل هامش ربح الأرقام إلى {val:.0f}%</b> بنجاح!",
                reply_markup=back_to("admin_pricing_manage"),
            )
        except ValueError:
            bot.reply_to(message, "❌ يرجى إدخال رقم صحيح (مثال: 25)")
        return
    ...
    if state == "admin_wait_ref_reward":
        ...
        try:
            val = float(text.replace("$", "").strip())
            DB["settings"]["referral_reward"] = round(val, 3)
            db_update()
            clear_state(message.from_user.id)
            bot.send_message(
                message.chat.id,
                f"✅ <b>تم تعديل مكافأة الإحالة إلى ${val:.2f}</b> بنجاح!",
                reply_markup=back_to("admin_pricing_manage"),
            )
        except ValueError:
            bot.reply_to(message, "❌ يرجى إدخال رقم صحيح (مثال: 0.10)")
        return

    # -----------------------------------------------------
    # Admin forced channel name
    # -----------------------------------------------------

    if state == "admin_wait_channel_name":
        if not is_admin(message.from_user.id):
            clear_state(message.from_user.id)
            return
        ...
        if not ch.startswith("@") and "t.me/" not in ch:
            ch = "@" + ch
        DB["settings"]["forced_channel"] = ch
        DB["settings"]["forced_channel_enabled"] = True
        db_update()
        clear_state(message.from_user.id)

        bot.send_message(
            message.chat.id,
            f"✅ <b>تم تعيين قناة الاشتراك الإجباري وتفعيلها:</b>\n<code>{safe_html(ch)}</code>\n\n"
            "تأكد من وجود البوت كمشرف في هذه القناة.",
            reply_markup=back_to("admin_maintenance_manage"),
        )
        return
    ...
    if state == "admin_broadcast":
        ...
        bot.send_message(
            message.chat.id,
            "📢 جاري إرسال الرسالة إلى جميع المستخدمين..."
        )
        ...
        bot.send_message(
            message.chat.id,
            "✅ <b>انتهت الإذاعة الجماعية</b>\n\n"
            f"📨 تم التسليم بنجاح: <b>{sent}</b>\n"
            f"❌ فشل (حظروا البوت): <b>{failed}</b>",
            reply_markup=admin_menu(),
        )
