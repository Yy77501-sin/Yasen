...
def get_or_create_user(user_id, username="", first_name=""):
    try:
        ...
        if row:
            ...
            conn.commit()
            conn.close()
            return curr_usd, curr_rub, int(row[2]), int(row[3]), str(row[4]), str(row[5])
        else:
...
def build_main_keyboard(user_id):
    ...
    # الصف 1: 🛍️ حسابات تيليجرام جاهزة | 📱 تفعيل أرقام SMS
    markup.add(
        types.InlineKeyboardButton("🛍️ حسابات تيليجرام جاهزة", callback_data="btn_ready_tg"),
        types.InlineKeyboardButton("📱 تفعيل أرقام SMS", callback_data="btn_buy_number")
    )
...
def start_command(message):
    try:
        ...
        text = (
            f"┏━━━━〔 🤖 **@FastNum1200_bot** 〕━━━━┓\n"
            f"👋 مرحباً بك يا **{first_name}** في المتجر الأقوى للأرقام والخدمات!\n\n"
            f"👤 • **معرفك (ID):** `{user_id}`\n"
            f"💰 • **رصيدك الحالي:** **{bal_usd:.2f} $** | **{bal_rub:.1f} ₽**\n"
            f"⚡ • **حالة السيرفرات:** **نشطة وجاهزة 100% 🟢**\n"
            f"┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛\n\n"
            f"👇 **يمكنك التحكم بالبوت عبر الأزرار في الأسفل:**"
        )
...
def router_callback(call):
    try:
        ...
        if data == "back_to_main":
            ...
            text = (
                f"┏━━━━〔 🤖 **@FastNum1200_bot** 〕━━━━┓\n"
                f"👋 مرحباً بك يا **{user_name}** في المتجر الأقوى للأرقام والخدمات!\n\n"
                f"👤 • **معرفك (ID):** `{user_id}`\n"
                f"💰 • **رصيدك الحالي:** **{bal_usd:.2f} $** | **{bal_rub:.1f} ₽**\n"
                f"⚡ • **حالة السيرفرات:** **نشطة وجاهزة 100% 🟢**\n"
                f"┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛\n\n"
