...
def sync_keys_between_db_and_config():
    ...
    try:
        for setting_k, var_n in SETTING_TO_CONFIG_VAR.items():
            ...
            cfg_val = getattr(config, var_n, os.getenv(var_n, "")).strip()
...
def add_custom_provider(name, api_url, api_key, provider_type="sms_activate"):
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        now_str = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
...
def get_fivesim_key():
    ...
    if not k:
        k = getattr(config, "FIVESIM_API_KEY", os.getenv("FIVESIM_API_KEY", "")).strip()
    return k
...
def get_grizzly_key():
    ...
    if not k:
        k = getattr(config, "GRIZZLY_API_KEY", os.getenv("GRIZZLY_API_KEY", "")).strip()
    return k
...
def get_hero_key():
    ...
    if not k:
        k = getattr(config, "HERO_SMS_API_KEY", getattr(config, "HERO_API_KEY", os.getenv("HERO_SMS_API_KEY", os.getenv("HERO_API_KEY", "")))).strip()
    return k
...
def custom_provider_request(provider_id_or_tuple, params):
    try:
        ...
        pid, name, api_url, api_key, p_type, is_act = p
...
def build_main_keyboard(user_id):
    markup = types.InlineKeyboardMarkup(row_width=2)
    # الصف 1: بدء طلبية رَشق جديدة
    markup.add(
        types.InlineKeyboardButton("بدء طلبية رَشق جديدة 🗃️", callback_data="btn_services_games")
    )
    # الصف 2: أرقام وهمية | شحن الألعاب
    markup.add(
        types.InlineKeyboardButton("أرقام وهمية 📇", callback_data="btn_buy_number"),
        types.InlineKeyboardButton("شحن الألعاب 🎁", callback_data="btn_games_charge")
    )
    # الصف 3: خدماتي المفضلة | اشتراكات برامج AI
    markup.add(
        types.InlineKeyboardButton("خدماتي المفضلة ★", callback_data="btn_favorites"),
        types.InlineKeyboardButton("اشتراكات برامج AI 🤖", callback_data="smm_sec_ai")
    )
    # الصف 4: شحن كرت | إشحن رصيدك
    markup.add(
        types.InlineKeyboardButton("شحن كرت 💳", callback_data="btn_redeem_card"),
        types.InlineKeyboardButton("إشحن رصيدك 👝", callback_data="btn_deposit")
    )
    # الصف 5: ارقام حسابات تليجرام
    markup.add(
        types.InlineKeyboardButton("ارقام حسابات تليجرام ✈️", callback_data="btn_ready_tg")
    )
    # الصف 6: المتجر الإلكتروني (بيع وشراء)
    markup.add(
        types.InlineKeyboardButton("المتجر الإلكتروني (بيع وشراء) ⚙️", callback_data="btn_market")
    )
    # الصف 7: مفتاح API
    markup.add(
        types.InlineKeyboardButton("مفتاح API 🌐", callback_data="btn_user_api")
    )
    # الصف 8: طلب تعويض | تغيير العملة
    markup.add(
        types.InlineKeyboardButton("طلب تعويض 📋", callback_data="btn_claim_compensation"),
        types.InlineKeyboardButton("تغيير العملة 🔄", callback_data="btn_change_currency")
    )
    # الصف 9: تحويل رصيد | رصيد مجاني
    markup.add(
        types.InlineKeyboardButton("تحويل رصيد 🔄", callback_data="btn_transfer_hub"),
        types.InlineKeyboardButton("رصيد مجاني ➕", callback_data="btn_free_points")
    )
    # الصف 10: المزيد والاعدادات
    markup.add(
        types.InlineKeyboardButton("المزيد والاعدادات ⚙️", callback_data="btn_more_settings")
    )
    # الصف 11: الدعم الفني
    markup.add(
        types.InlineKeyboardButton("الدعم الفني 👤", url=get_support_url())
    )
    
    if str(user_id) == str(ADMIN_ID):
        markup.add(types.InlineKeyboardButton("👑 لوحة تحكم الإدارة الشاملة", callback_data="btn_admin_panel"))
    return markup
...
def start_command(message):
    try:
        ...
        text = (
            ...
            f"⚡ • **سيرفرات التفعيل:** **سيرفرات فائقة السرعة نشطة 100% 🟢**\n"
...
def handle_all_text_messages(message):
    try:
        ...
        # ----------------- 5. إدارة الشحن اليدوي والخصم -----------------
        ...
        elif action == "admin_input_cprov_key" and user_id_str == str(ADMIN_ID):
        ...
        elif action == "admin_add_prov_name" and user_id_str == str(ADMIN_ID):
            ...
            safe_reply_to(
                ...
                f"📝 **الخطوة 2 من 3:**\n"
                f"أرسل الآن **رابط الـ API للموقع** (Handler API URL):\n"
                f"• مثال لمواقع بروتوكول SMS-Activate:\n"
                f"`https://sms-activate.org/stubs/handler_api.php`"
            )

        elif action == "admin_add_prov_url" and user_id_str == str(ADMIN_ID):
            prov_url = text.strip()
            prov_name = state.get("name", "Custom Provider")
            ...
            safe_reply_to(
                ...
                f"أرسل الآن **مفتاح الـ API (API Key)** الخاص بحسابك في هذا الموقع:"
            )

        elif action == "admin_add_prov_key" and user_id_str == str(ADMIN_ID):
            prov_key = text.strip()
            prov_name = state.get("name", "Custom Provider")
            prov_url = state.get("url", "")
            new_id = add_custom_provider(prov_name, prov_url, prov_key, "sms_activate")
            ...
            safe_reply_to(
                message,
                f"🎉 **تمت إضافة المزود بنجاح وأصبح متاحاً للشراء في البوت!**\n"
                f"━━━━━━━━━━━━━━━━━━\n"
                f"🏷️ • الاسم: **{prov_name}**\n"
                f"🔗 • الرابط: `{prov_url}`\n"
...
def router_callback(call):
    try:
        ...
        # ----------------- قسم أرقام تيليجرام الجاهزة (موقع TG) -----------------
            ...
            markup.add(
                types.InlineKeyboardButton("💬 عروض واتساب", callback_data="btn_offers_wa"),
            ...
            markup = types.InlineKeyboardMarkup(row_width=1)
            markup.add(
                types.InlineKeyboardButton("🟢 رقم واتساب فوري (السيرفر 1)", callback_data="srv_wa"),
                types.InlineKeyboardButton("✈️ رقم تيليجرام فوري (السيرفر 1)", callback_data="srv_tg"),
                types.InlineKeyboardButton("✈️ حسابات تيليجرام جاهزة (تسليم فوري)", callback_data="btn_ready_tg"),
                types.InlineKeyboardButton("👑 رشق متابعين تيك توك فوري", callback_data="smm_sec_tt"),
        ...
        # ----------------- لوحة تحكم الإدارة -----------------
            ...
            if not p:
                ...
                return
            pid, pname, purl, pkey, ptype, pact = p
            success, bal_info = custom_provider_get_balance(pid)
            status_str = f"✅ متصل! الرصيد: `{bal_info}`" if success else f"⚠️ رد المزود: `{bal_info}`"
            
            pkey_disp = (pkey[:15] + "...") if len(pkey) > 15 else pkey
            panel_text = (
                f"⚡ **إعدادات وتشخيص المزود: {pname}**\n"
                f"━━━━━━━━━━━━━━━━━━\n"
                f"🔗 • **رابط الـ API:** `{purl}`\n"
                f"🔑 • **مفتاح الـ API:** `{pkey_disp}`\n"
                f"📊 • **فحص الرصيد والاتصال:**\n{status_str}\n"
                f"━━━━━━━━━━━━━━━━━━\n"
                f"👇 يمكنك تحديث المفتاح أو حذف المزود:"
            )
            cp_markup = types.InlineKeyboardMarkup(row_width=1)
            cp_markup.add(
                types.InlineKeyboardButton("🔑 تعديل مفتاح API المزود", callback_data=f"adm_edit_cprov_key_{pid}"),
                types.InlineKeyboardButton("🔄 إعادة فحص الرصيد والاتصال", callback_data=f"adm_custom_prov_{pid}"),
                types.InlineKeyboardButton("🗑️ حذف هذا المزود نهائياً", callback_data=f"adm_del_cprov_{pid}"),
            ...
            bot.answer_callback_query(call.id, "✅ تم حذف المزود بنجاح!", show_alert=True)
            ...
            fs_display = (fs_k[:20] + "..." + fs_k[-10:]) if len(fs_k) > 30 else (fs_k if fs_k else "غير مدخل")
            hero_display = (hero_k[:15] + "..." + hero_k[-8:]) if len(hero_k) > 25 else (hero_k if hero_k else "غير مدخل")
            tg_display = (tg_k[:12] + "...") if len(tg_k) > 12 else (tg_k if tg_k else "غير مدخل")

            text = (
                f"🔑 **لوحة إدارة المزودين ومفاتيح الـ API:**\n"
                f"━━━━━━━━━━━━━━━━━━\n"
                f"🌐 • **مزود 5SIM العالمي (السيرفر 1):**\n`{fs_display}`\n\n"
                f"💳 • **مزود Hero SMS (السيرفر 3):**\n`{hero_display}`\n\n"
                f"✈️ • **مزود TG-Lion (أرقام وحسابات تيليجرام):**\n`{tg_display}`\n"
                f"━━━━━━━━━━━━━━━━━━\n"
                f"👇 اضغط على أي مزود لفحص رصيده أو تعديل المفتاح الخاص به:"
            )
            markup = types.InlineKeyboardMarkup(row_width=1)
            markup.add(
                types.InlineKeyboardButton("🌐 إعدادات وفحص مزود 5SIM (السيرفر 1)", callback_data="admin_settings_fivesim"),
                types.InlineKeyboardButton("💳 إعدادات وفحص مزود Hero SMS (السيرفر 3)", callback_data="admin_settings_hero"),
                types.InlineKeyboardButton("🐻 إعدادات وفحص مزود Grizzly SMS (السيرفر 2)", callback_data="admin_settings_grizzly"),
                types.InlineKeyboardButton("✈️ إعدادات مزود TG-Lion (حسابات جاهزة)", callback_data="admin_check_tg")
            )
            
            # عرض المزودات المضافة يدوياً
            cust_provs = get_all_custom_providers()
            if cust_provs:
                for cp in cust_provs:
                    c_id, c_name, c_url, c_key, c_type, c_act = cp
                    markup.add(types.InlineKeyboardButton(f"⚡ مزود مضاف: {c_name} (فحص وتعديل)", callback_data=f"adm_custom_prov_{c_id}"))
                    
            markup.add(
                types.InlineKeyboardButton("➕ إضافة موقع مزود جديد للأرقام (Add Provider)", callback_data="admin_add_provider_step1"),
                types.InlineKeyboardButton("🔙 العودة للوحة الإدارة", callback_data="btn_admin_panel")
            )
            safe_edit_message_text(chat_id, call.message.message_id, text, reply_markup=markup)
