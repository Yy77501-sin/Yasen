...
def is_admin_user(user_id):
    ...
    try:
        ...
        if db_adm and u == db_adm:
            return True
        cfg_adm = str(getattr(config, "ADMIN_ID", os.getenv("ADMIN_ID", ""))).strip()
...
def build_main_keyboard(user_id):
    ...
    # الصف 1: شراء الأرقام (زر عريض بارز في القمة مثل تصميم الصورة)
    markup.add(
        types.InlineKeyboardButton("📱 شراء الأرقام", callback_data="btn_buy_number")
    )
    
    # الصف 2: أرقام جاهزة | خدمات الرشق
    markup.add(
    ...
    # الصف 4: تحويل الرصيد | حسابي ومحفظتي
    markup.add(
    ...
    # الصف 5: حالة السيرفرات (زر عريض في المنتصف)
    markup.add(
        types.InlineKeyboardButton("📊 السيرفرات", callback_data="btn_stats")
    )
    
    # الصف 6: رصيد مجاني | تعليمات واستخدام
    markup.add(
        types.InlineKeyboardButton("🎁 رصيد مجاني", callback_data="btn_free_points"),
        types.InlineKeyboardButton("📚 تعليمات واستخدام", callback_data="btn_instructions")
    )
    
    # الصف 7: القناة الرسمية | الدعم الفني
    btn_ch = get_safe_url_button("📢 القناة الرسمية", MAIN_CHANNEL_URL, "btn_main_channel")
    btn_sup = types.InlineKeyboardButton("💬 الدعم الفني", url=get_support_url())
    markup.add(btn_ch, btn_sup)
    
    # الصف 8: لوحة تحكم الإدارة للأدمن
    if is_admin_user(user_id):
        markup.add(types.InlineKeyboardButton("👑 لوحة تحكم الإدارة", callback_data="btn_admin_panel"))
    ...
    try:
        ...
        # ----------------- لوحة تحكم الإدارة -----------------
            ...
            try:
                ...
                print(f"Custom providers display error: {e}")
