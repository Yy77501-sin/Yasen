...
def build_tg_ready_page_keyboard(page=0, query=""):
    ...
    page_size = 20  # 10 صفوف في عمودين = 20 دولة في الصفحة
    ...
    nav_row = []
    if page > 0:
        nav_row.append(types.InlineKeyboardButton("➡️ السابق", callback_data=f"page_tg_ready_{page-1}"))
        
    nav_row.append(types.InlineKeyboardButton(f"📄 {page+1} / {total_pages}", callback_data="noop"))
    
    if page < total_pages - 1:
        nav_row.append(types.InlineKeyboardButton("التالي ⬅️", callback_data=f"page_tg_ready_{page+1}"))
    ...
    if query:
        markup.row(
            types.InlineKeyboardButton("🔍 بحث جديد", callback_data="prompt_search_tg_ready"),
            types.InlineKeyboardButton("🔄 كل الدول", callback_data="btn_ready_tg")
        )
    else:
        markup.row(
            types.InlineKeyboardButton("🔍 بحث عن دولة", callback_data="prompt_search_tg_ready"),
            types.InlineKeyboardButton("🔄 تحديث المخزون", callback_data="btn_ready_tg")
        )
...
def router_callback(call):
    try:
        ...
        # ----------------- مسارات تصفح التطبيقات والسيرفرات -----------------
            ...
            try:
                bot.answer_callback_query(call.id, "🔄 جاري تحديث قائمة الحسابات والمخزون الحي...")
            except Exception:
                pass
            if data == "btn_ready_tg":
            ...
            if data.startswith("page_tg_ready_"):
                try:
                    ...
                    page = 0
            tg_text = (
                "✈️ **حسابات TELEGRAM جاهزة (موقع TG)**\n"
                "━━━━━━━━━━━━━━━━━\n"
                "🌟 تسليم فوري وتلقائي لبيانات الجلسة (Session / Tdata).\n"
        ...
        # ----------------- لوحة تحكم الإدارة -----------------
            ...
            text = (
                f"🔑 **إدارة المزودين ومفاتيح الـ API:**\n"
                f"━━━━━━━━━━━━━━━━━━\n"
                f"🌐 • **مزود 5SIM العالمي (السيرفر المعتمد 1):**\n`{fs_k[:25]}...{fs_k[-15:] if len(fs_k)>40 else ''}`\n\n"
                f"✈️ • **مزود TG-Lion (حسابات جاهزة):**\n`{tg_k[:10]}...`\n"
                f"━━━━━━━━━━━━━━━━━━\n"
                f"👇 اختر المزود لفحص اتصاله وتعديل مفتاحه مباشرة:"
            )
            markup = types.InlineKeyboardMarkup(row_width=1)
            markup.add(
                types.InlineKeyboardButton("🌐 سيرفر 5SIM العالمي", callback_data="admin_settings_fivesim"),
                types.InlineKeyboardButton("✈️ مزود TG-Lion (حسابات)", callback_data="admin_check_tg"),
                types.InlineKeyboardButton("🔙 العودة للوحة الإدارة", callback_data="btn_admin_panel")
            )
            safe_edit_message_text(chat_id, call.message.message_id, text, reply_markup=markup)
