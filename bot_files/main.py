...
def fivesim_buy_number(country_name, service_name):
    try:
        ...
        res = session.get(url, headers=fivesim_headers(), timeout=9)
        if res.status_code == 200:
            data = res.json()
            return True, {
                "id": str(data.get("id")),
            ...
            return False, "رصيد السيرفر غير كافٍ في 5SIM."
        return False, f"رد 5SIM ({res.status_code}): {res.text[:120]}"
    except Exception as e:
        return False, str(e)
...
def get_hero_key():
    ...
    if not k or len(k) < 5:
        k = "Ac7cd39c1650e6eb66d1A3e903c4A58A"
    return k
    ...
    for endpoint in HERO_ENDPOINTS:
        try:
            ...
            if res.status_code == 200 and res.text.strip():
                ...
                return "⚠️ مفتاح Hero SMS غير مفعل أو مرفوض من المزود (Permission denied)"
        except Exception:
            continue
    return "ERROR_CONNECTION"
...
def get_country_info_dynamic(c_id):
    ...
    clean = c_id_str.lower().replace(" ", "").replace("_", "")
...
def get_live_countries_for_server(provider_name, app_code):
            ...
            if isinstance(data, dict):
                for cid_str, srv_map in data.items():
                    ...
                    if not isinstance(p_info, dict):
                        continue
                    try:
                        raw_cost_rub = float(list(p_info.keys())[0])
                        count_val = int(list(p_info.values())[0])
                    except Exception:
                        continue
    ...
    if live_list:
        # ترتيب الدول: الأكثر توفراً ثم الأرخص
        live_list.sort(key=lambda x: (-1 if x['count'] > 0 else 1, x['price_usd']))
        live_countries_cache[cache_key] = {"data": live_list, "time": now_ts}
        return live_list

    # في حالة الطوارئ أو تعثر استجابة المزود نعود للقائمة الكاملة
    fallback_list = []
    for cid, cinfo in catalog.COUNTRIES.items():
...
def get_live_tg_ready_catalog():
    now_ts = time.time()
    if "data" in tg_catalog_cache and now_ts - tg_catalog_cache.get("time", 0) < 60:
    ...
    try:
        ...
        if data and data.get("status") == "ok" and "countries" in data and isinstance(data["countries"], dict):
            ...
            for code, c_data in data["countries"].items():
                ...
                # ترجمة الأسماء إلى العربية إن أمكن
                clean_name = c_name.split()[0].lower().replace(" ", "").replace("_", "")
                tr_info = catalog.GLOBAL_COUNTRY_TRANSLATIONS.get(clean_name)
                country_display = f"{tr_info['title']} {tr_info['flag']}" if tr_info else c_name
                
                acc_key = f"tglion_{code.lower()}"
                res_dict[acc_key] = {
                    "id": acc_key,
                    "code": code.lower(),
                    "code_num": code_num,
    ...
    return catalog.TG_READY_ACCOUNTS
...
def build_tg_ready_page_keyboard(page=0, query=""):
    ...
    if query:
        q = query.lower().strip()
        items = [it for it in items if q in it[1]["country"].lower() or q in it[1].get("raw_name", "").lower() or q in it[1]["desc"].lower() or q in it[1].get("code", "").lower()]
    ...
    for acc_id, acc_info in batch:
        lbl = f"{acc_info['country']} • {acc_info['price_usd']:.2f}$ ({acc_info['type']})"
        markup.add(types.InlineKeyboardButton(lbl, callback_data=f"tg_acc_buy_{acc_id}"))
        
    nav_buttons = []
    if page > 0:
        nav_buttons.append(types.InlineKeyboardButton("➡️ السابق", callback_data=f"page_tg_ready_{page-1}"))
    nav_buttons.append(types.InlineKeyboardButton(f"📄 {page+1}/{total_pages}", callback_data="noop"))
    if page < total_pages - 1:
        nav_buttons.append(types.InlineKeyboardButton("التالي ⬅️", callback_data=f"page_tg_ready_{page+1}"))
        
    if nav_buttons:
        markup.row(*nav_buttons)
        
    markup.add(types.InlineKeyboardButton("🔍 🚀 البحث عن دولة في موقع TG", callback_data="prompt_search_tg_ready"))
...
def build_countries_page_keyboard(app_code, server_key, page=0):
    ...
    for c in current_batch:
        ...
        buttons.append(types.InlineKeyboardButton(btn_text, callback_data=f"card_{app_code}_{server_key}_{c['id']}_{page}"))
    markup.add(*buttons)
    ...
    try:
        ...
        # ----------------- 4. البحث الديناميكي عن دولة -----------------
            ...
            if not matched_countries:
                # فحص الكتالوج الموسع في حال لم تكن في الكاش
                for cid, cinfo in catalog.COUNTRIES.items():
                    if (search_query in cinfo["name"].lower() or 
                        search_query in cinfo["title"].lower() or 
                        search_query in cinfo.get("prefix", "") or 
                        search_query == cid):
                        p_rub, p_usd = get_server_price(provider_name, app_code, "other", cid, cinfo["name"])
