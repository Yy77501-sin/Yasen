...
def get_live_tg_ready_catalog():
    now_ts = time.time()
    if "data" in tg_catalog_cache and now_ts - tg_catalog_cache.get("time", 0) < 120:
        ...
        if data and data.get("status") == "ok" and "countries" in data and isinstance(data["countries"], dict):
            ...
            if raw_dict:
                ...
                def sort_key(item):
                    c_code = item[1]["code"]
                    qty_val = item[1]["qty"]
                    # إذا كانت الدولة في قائمة الأولويات
                    prio_idx = PRIORITY_COUNTRY_CODES.index(c_code) if c_code in PRIORITY_COUNTRY_CODES else 999
                    # الدول التي بها كمية > 0 تأتي أولاً، ثم الأولوية، ثم الكمية التنازلية
...
def build_tg_ready_page_keyboard(page=0, query=""):
    ...
    for acc_id, acc_info in batch:
        stock_str = f"({acc_info['qty']} متاح)" if acc_info['qty'] > 0 else "(نفذ)"
        lbl = f"{acc_info['country']} • {acc_info['price_usd']:.2f}$ {stock_str}"
...
def buy_server_number(provider_name, app_info, c_info, country_id):
    ...
    if provider_name == "fivesim":
        ...
        if resp.startswith("ACCESS_NUMBER:"):
            ...
            return False, "الأرقام لهذه الدولة غير متوفرة حالياً في هذا السيرفر (نفذت الكمية المؤقتة)."
        elif resp == "NO_BALANCE":
            return False, "رصيد السيرفر لدى المزود غير كافٍ حالياً."
        elif "FORBIDDEN" in resp or "Permission denied" in resp:
            return False, "سيرفر التزويد غير مصرح به حالياً أو مفتاح الـ API غير مفعل (FORBIDDEN). يرجى تجربة سيرفر آخر."
        elif "BAD_KEY" in resp or "ERROR_KEY" in resp:
            return False, "مفتاح الربط بالسيرفر غير صالح أو منتهي الصلاحية."
        elif "WRONG_SERVICE" in resp:
            return False, "هذه الخدمة غير مدعومة لهذه الدولة حالياً في هذا السيرفر."
        elif resp == "ERROR_CONNECTION":
            return False, "تعذر الاتصال بسيرفر المزود، يرجى إعادة المحاولة."
        return False, f"رد المزود: {resp[:120]}"
