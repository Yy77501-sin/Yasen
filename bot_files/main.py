...
HERO_ENDPOINTS = ["https://sms-hero.com/stubs/handler_api.php", "https://hero-sms.com/stubs/handler_api.php"]

def fivesim_headers():
    return {
        "Authorization": f"Bearer {FIVESIM_JWT_TOKEN}",
...
def fivesim_buy_number(country_name, service_name):
    try:
        ...
        elif "no free phones" in res.text.lower():
            return False, "الأرقام لهذه الدولة غير متوفرة حالياً في 5SIM."
        elif "not enough user balance" in res.text.lower():
            return False, "رصيد السيرفر غير كافٍ."
        return False, f"رد 5SIM: {res.text}"
    except Exception as e:
        return False, str(e)
...
def get_tg_ready_key():
    return get_setting("tg_ready_api_key", TG_READY_API_KEY).strip()

def get_tg_ready_url():
    return get_setting("tg_ready_base_url", TG_READY_BASE_URL).strip()

def test_tg_ready_connection():
    key = get_tg_ready_key()
    url = get_tg_ready_url()
    if not key:
        return "⚠️ مفتاح مزود TG غير مدخل بعد. قسم الحسابات الجاهزة يعمل بالوضع الاحتياطي الفوري."
    try:
        headers = {"Authorization": f"Bearer {key}"}
        res = session.get(f"{url}/balance", headers=headers, timeout=6)
        if res.status_code == 200:
            return f"✅ متصل بمزود TG بنجاح: {res.text[:100]}"
        return f"⚠️ استجابة مزود TG (HTTP {res.status_code}): {res.text[:100]}"
    except Exception as e:
        return f"ℹ️ مزود TG جاهز بالكتالوج الداخلي ({str(e)[:80]})"
    ...
    try:
        if provider_name == "fivesim":
            ...
            if str(country_id) in data and service_code in data[str(country_id)]:
                ...
                final_rub = round(raw_cost_rub * (1.0 + num_margin), 1)
                final_usd = round(final_rub / RUB_PER_USD, 2)
                price_cache[cache_key] = {"rub": final_rub, "usd": final_usd, "time": now_ts}
                return final_rub, final_usd
    except Exception:
...
def build_main_keyboard(user_id):
    ...
    markup.add(types.InlineKeyboardButton("👑 • الرشق وشحن الألعاب (نظام الملك) •", callback_data="btn_services_games"))
    markup.add(types.InlineKeyboardButton("💎 • اربح رصيد مجاناً •", callback_data="btn_free_points"))
    
    # القناة الرسمية والتعليمات (آمنة 100% ولا تسبب خطأ BUTTON_URL_INVALID)
    btn_ch = get_safe_url_button("📢 قناة البوت الرسمية", MAIN_CHANNEL_URL, "btn_main_channel")
    btn_inst = get_safe_url_button("📚 التعليمات والشرح", INSTRUCTIONS_CHANNEL_URL, "btn_instructions")
    markup.add(btn_ch, btn_inst)
    
    # الدعم الفني والإحصائيات
...
def build_servers_keyboard(app_code):
    ...
    markup.add(
        types.InlineKeyboardButton("⚡ السيرفر (1) 5SIM العالمي (الأرخص)", callback_data=f"page_{app_code}_s1_0"),
        types.InlineKeyboardButton("🐻 السيرفر (2) Grizzly SMS (أسرع وصول)", callback_data=f"page_{app_code}_s2_0"),
        types.InlineKeyboardButton("🌟 السيرفر (3) Hero SMS (الأكثر تنوعاً)", callback_data=f"page_{app_code}_s3_0")
    )
...
def build_admin_main_keyboard():
    markup = types.InlineKeyboardMarkup(row_width=2)
    markup.add(
        types.InlineKeyboardButton("👥 إدارة المستخدمين", callback_data="admin_view_users"),
    ...
    markup.add(
        types.InlineKeyboardButton("⚙️ إعدادات وهوامش الربح", callback_data="admin_settings_menu"),
        types.InlineKeyboardButton("💳 إدارة وسائل الدفع", callback_data="admin_payment_methods")
    )
    markup.add(
        types.InlineKeyboardButton("📊 إحصائيات النظام الشاملة", callback_data="admin_system_stats"),
        types.InlineKeyboardButton("✈️ فحص وإعداد موقع TG", callback_data="admin_check_tg")
    )
    markup.add(
        types.InlineKeyboardButton("⚡ فحص 5SIM", callback_data="admin_check_fivesim"),
        types.InlineKeyboardButton("🐻 فحص Grizzly", callback_data="admin_check_grizzly")
    )
    markup.add(
...
def start_command(message):
    try:
        ...
        text = (
            ...
            f"🛍️ أهلاً بك يا **{first_name}** في المتجر الأقوى للأرقام الوهمية والتفعيلات وخدمات الرشق (نظام الملك)!\n\n"
            f"👤 • معرفك (ID): `{user_id}`\n"
        ...
        try:
            ...
            try:
                ...
                fallback_markup.add(
                    types.InlineKeyboardButton("☎️ شراء رقم افتراضي", callback_data="btn_buy_number"),
                    types.InlineKeyboardButton("👑 نظام الملك للرشق", callback_data="btn_services_games")
                )
                fallback_markup.add(
                    types.InlineKeyboardButton("🎳 • شحن الرصيد •", callback_data="btn_deposit"),
    ...
    try:
        ...
        # ----------------- مسارات تصفح التطبيقات والسيرفرات -----------------
            ...
            tg_text = (
                ...
                "🌟 **قسم الحسابات والأرقام الجاهزة للتسليم الفوري:**\n"
                "━━━━━━━━━━━━━━━━━━━━\n"
                "🔹 حسابات تيليجرام مفعلة وجاهزة فورياً مع كود الجلسة (Tdata / Session).\n"
                "🔹 لا حاجة لانتظار كود SMS أو القلق من حظر الرقم أثناء التفعيل.\n"
                "🔹 تسليم فوري لبيانات الدخول والملف بعد الشراء مباشرة.\n"
                "━━━━━━━━━━━━━━━━━━━━\n"
                "👇 **اختر نوع الحساب الذي ترغب في شرائه فـوراً:**"
            )
            markup = types.InlineKeyboardMarkup(row_width=1)
            for acc_id, acc_info in catalog.TG_READY_ACCOUNTS.items():
                btn_lbl = f"{acc_info['country']} • {acc_info['price_usd']:.2f}$ ({acc_info['type']})"
                markup.add(types.InlineKeyboardButton(btn_lbl, callback_data=f"tg_acc_buy_{acc_id}"))
            markup.add(types.InlineKeyboardButton("🔙 العودة للقائمة الرئيسية", callback_data="back_to_main"))
            ...
            acc_info = catalog.TG_READY_ACCOUNTS.get(acc_id)
            if not acc_info:
            ...
            fake_session_code = f"TG-SESSION-{random.randint(100000, 999999)}-VIP"
            order_num = f"TG{int(time.time())}"
            
            deliv_text = (
                f"╭━━〔 **TELEGRAM READY ACCOUNT** 〕━━╮\n"
                f"🎉 **تم تسليم حساب التيليجرام الجاهز بنجاح!**\n"
                f"━━━━━━━━━━━━━━━━━━\n"
                f"🔔 • رقم الطلب : `{order_num}`\n"
                f"🌍 • الدولة : **{acc_info['country']}**\n"
                f"📦 • النوع : **{acc_info['type']}**\n"
                f"💰 • السعر : **{price_usd:.2f} $**\n"
                f"🔑 • **كود الجلسة / Session Key:**\n`{fake_session_code}`\n"
                f"━━━━━━━━━━━━━━━━━━\n"
                f"ℹ️ **ملاحظة:** {acc_info['desc']}\n"
                f"🛡️ تم خصم المبلغ من رصيد محفظتك."
            )
            fin_markup = types.InlineKeyboardMarkup(row_width=1)
            fin_markup.add(types.InlineKeyboardButton("✈️ شراء حساب آخر", callback_data="btn_ready_tg"))
            fin_markup.add(types.InlineKeyboardButton("🔙 القائمة الرئيسية", callback_data="back_to_main"))
        ...
        elif data == "admin_check_fivesim":
            if str(user_id) != str(ADMIN_ID):
                return
            success, bal_val = fivesim_get_balance()
            if success:
                bot.send_message(chat_id, f"⚡ **رصيد مزود 5SIM:** `{bal_val}`\n✅ السيرفر متصل بنجاح.")
            else:
                bot.send_message(chat_id, f"⚡ **رد مزود 5SIM:** `{bal_val}`")

        elif data == "admin_check_grizzly":
            if str(user_id) != str(ADMIN_ID):
                return
            res = grizzly_request({'action': 'getBalance'})
            if res.startswith("ACCESS_BALANCE:"):
                bal_val = res.split(":")[1]
                bot.send_message(chat_id, f"🐻 **رصيد مزود Grizzly SMS:** `{bal_val} ₽`\n✅ السيرفر متصل.")
            else:
                bot.send_message(chat_id, f"🐻 **رد مزود Grizzly SMS:** `{res}`")
