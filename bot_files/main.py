...
def build_main_keyboard(user_id):
    ...
    # الصف 1: شراء حسابات تيليجرام الجاهزة (تسليم فوري لكود الدخول والملفات)
    markup.add(
        types.InlineKeyboardButton("✈️ شراء حسابات تيليجرام جاهزة (تسليم فوري)", callback_data="btn_ready_tg")
    )
    
    # الصف 2: أرقام تيليجرام لتفعيل كود SMS | شحن الرصيد
    markup.add(
        types.InlineKeyboardButton("📱 تفعيل كود SMS تيليجرام", callback_data="srv_tg"),
        types.InlineKeyboardButton("💳 شحن الرصيد", callback_data="btn_deposit")
    )
    
    # الصف 3: حسابي ومحفظتي | تحويل الرصيد
    markup.add(
    ...
    # الصف 4: الدعم الذكي (AI) | رصيد مجاني
    markup.add(
    ...
    # الصف 5: حالة السيرفرات | خدمات الرشق
    markup.add(
        types.InlineKeyboardButton("📊 حالة السيرفرات", callback_data="btn_stats"),
        types.InlineKeyboardButton("👑 خدمات الرشق", callback_data="btn_services_games")
    )
    
    # الصف 6: دليل تسجيل الدخول | القناة الرسمية
    btn_ch = get_safe_url_button("📢 القناة الرسمية", MAIN_CHANNEL_URL, "btn_main_channel")
    btn_inst = types.InlineKeyboardButton("📚 دليل تسجيل الدخول", callback_data="btn_instructions")
    markup.add(btn_inst, btn_ch)
