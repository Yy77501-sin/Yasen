...
def get_tg_ready_key():
    k = get_setting("tg_ready_api_key", "").strip()
    if k:
        return k
    if db:
        try:
            prov = db.get_provider(1)
            if prov and prov.get("api_key") and not prov.get("api_key", "").startswith("tg_live_sec_"):
                return prov.get("api_key").strip()
            all_p = db.get_all_providers(only_enabled=False)
            for p in all_p:
                if (p.get("adapter") == "tg_lion" or "lion" in p.get("name", "").lower()) and p.get("api_key"):
                    if not p.get("api_key", "").startswith("tg_live_sec_"):
                        return p.get("api_key").strip()
        except Exception as e:
...
def get_tg_ready_url():
    ...
    if db:
        try:
            prov = db.get_provider(1)
            if prov and prov.get("api_url"):
                return prov.get("api_url").strip()
        except Exception:
            pass
    return TG_READY_BASE_URL.strip() or "https://TG-Lion.net"
