...
BOT_NAME = os.getenv("BOT_NAME", "YasNum").strip() or "YasNum"
SUPPORT_USERNAME = os.getenv("SUPPORT_USERNAME", "@Yy77501").strip()
...
BANK_INFO = os.getenv(
    "BANK_INFO",
    "يرجى التواصل مع الدعم للحصول على بيانات التحويل البنكي."
).strip()

WALLET_INFO = os.getenv(
    "WALLET_INFO",
    "يرجى التواصل مع الدعم للحصول على بيانات المحفظة."
).strip()

DB_FILE = os.getenv("YASNUM_DB_FILE", "yasnum_data.json")
...
if ":" not in BOT_TOKEN:
    raise RuntimeError(
        "BOT_TOKEN looks invalid. Telegram bot tokens normally contain ':'."
    )
...
def main_menu(user_id=None):
    ...
    markup.row(
        ...
        types.InlineKeyboardButton("💳 شحن رصيدك", callback_data="recharge"),
    )
...
def welcome_text():
    return (
        ...
        "🚀 منصتك للخدمات الرقمية والأرقام الافتراضية\n"
        "⚡ تنفيذ سريع\n"
        "💰 محفظة ورصيد\n"
        "🎁 مكافآت إحالة\n\n"
...
def show_countries(chat_id, message_id=None):
    ...
    if error:
        ...
        text = (
            "☎️ <b>شراء رقم افتراضي</b>\n\n"
            "🌍 اختر الدولة:\n\n"
            f"📌 الدول المتاحة: <b>{len(countries)}</b>"
        )
