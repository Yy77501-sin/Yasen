...
try:
    ...
    PLUS_API_KEY = os.getenv('PLUS_API_KEY', '').strip()
...
PLUS_ENDPOINTS = ["https://sms-plus.net/stubs/handler_api.php"]
...
def get_server_price(provider_name, service_code, fivesim_code, country_id, country_name):
    ...
    try:
        if provider_name == "fivesim":
            ...
            elif provider_name == "plus":
                resp = plus_request({'action': 'getPrices', 'service': service_code, 'country': str(country_id)})
            else:
        ...
        if resp.startswith("ACCESS_NUMBER:"):
            ...
            if expected_prefix and not phone.startswith(expected_prefix):
                if provider_name == "grizzly":
                    ...
                    plus_request({'action': 'setStatus', 'id': order_id, 'status': '8'})
                else:
...
def set_server_status(provider_name, order_id, status_code):
    if provider_name == "fivesim":
        ...
        if provider_name == "grizzly":
            ...
            threading.Thread(target=plus_request, args=(params,), daemon=True).start()
        else:
    ...
    while time.time() - start_time < max_duration:
        ...
        try:
            if provider_name == "fivesim":
                ...
                resp = plus_request({'action': 'getStatus', 'id': order_id})
            else:
...
def build_servers_keyboard(app_code):
    ...
    markup.add(
        ...
        types.InlineKeyboardButton("💎 السيرفر (4) Plus SMS (الاقتصادي)", callback_data=f"page_{app_code}_s4_0")
    )
...
def build_admin_main_keyboard():
    ...
    markup.add(
        ...
        types.InlineKeyboardButton("💎 فحص وإعداد Plus VIP", callback_data="admin_check_plus")
    )
    markup.add(
