...
def build_main_keyboard(user_id):
    ...
    # الصف 2: شحن الرصيد | حسابي ومحفظتي
    markup.add(
...
def router_callback(call):
    try:
        ...
        # ----------------- لوحة تحكم الإدارة -----------------
            ...
            try:
                bot.answer_callback_query(call.id, "🔑 جاري فتح إدارة مزودي حسابات تيليجرام...")
            except Exception:
                pass
            if not is_admin_user(user_id):
                try:
                ...
                return
            try:
                tg_url = get_tg_ready_url()
                tg_k = get_tg_ready_key()
                tg_display = (tg_k[:12] + "...") if len(tg_k) > 12 else (tg_k if tg_k else "غير مدخل")

                text = (
                    "🔑 **لوحة إدارة مزودي حسابات وأرقام تيليجرام الجاهزة:**\n"
                    "━━━━━━━━━━━━━━━━━━\n"
                    f"✈️ • **المزود الرئيسي (TG Ready Server):**\n"
                    f"🔗 الرابط: `{tg_url}`\n"
                    f"🔑 المفتاح: `{tg_display}`\n"
                    "━━━━━━━━━━━━━━━━━━\n"
                    "👇 اختر المزود لفحص رصيده أو إضافة مزود جديد:"
                )
                markup = types.InlineKeyboardMarkup(row_width=1)
                markup.add(
                    types.InlineKeyboardButton("✈️ فحص وتعديل المزود الرئيسي (TG Ready)", callback_data="admin_check_tg")
                )
                
                # عرض المزودات المضافة
                try:
                ...
                markup.add(
                    types.InlineKeyboardButton("➕ إضافة موقع مزود جديد لحسابات TG", callback_data="admin_add_provider_step1"),
                    types.InlineKeyboardButton("🔙 العودة للوحة الإدارة", callback_data="btn_admin_panel")
                )
                safe_edit_message_text(chat_id, call.message.message_id, text, reply_markup=markup)
            except Exception as e:
