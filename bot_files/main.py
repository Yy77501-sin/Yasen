...
try:
    import config
    BOT_TOKEN = getattr(config, 'BOT_TOKEN', '8998307482:AAGW3woMPiD5fuMnQuCvQh6z_2GTa5Wsu6w').strip()
    FIVESIM_JWT_TOKEN = getattr(config, 'FIVESIM_JWT_TOKEN', '').strip()
    ...
    FIVESIM_JWT_TOKEN = os.getenv("FIVESIM_JWT_TOKEN", "").strip()
...
print(f"🚀 Initializing Number SMS Bot with King SMM System & 30% Profit Engine...")
        ...
        if provider_name == "fivesim":
            ...
            if res.status_code == 200:
                ...
                if country_name in data and fivesim_code in data[country_name]:
                    ...
                    # إضافة 30% هامش ربح
                    final_rub = round(cheapest * (1.0 + PROFIT_MARGIN), 1)
