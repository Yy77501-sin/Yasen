        ...
        # ----------------- لوحة تحكم الإدارة -----------------
            ...
            try:
                ...
                for idx, adapter in enumerate(active_provs):
                    ...
                    st = getattr(adapter, "status", "online")
                    st_emoji = "🟢" if st == "online" else ("🟡" if st == "degraded" else "🔴")
                    masked = mask_api_key(getattr(adapter, "api_key", "")) if 'mask_api_key' in globals() else "••••••••"
                    bal = getattr(adapter, "current_balance", 0.0)
                    bal_str = f"${bal:.2f}"
                    prio = getattr(adapter, "priority", 100)
                    min_b = getattr(adapter, "min_balance", 10.0)
                    atype = getattr(adapter, "adapter_type", "standard")
                    lines_summary.append(
                        f"{st_emoji} {medal} **{adapter.name}**\n"
                        f"   • الأولوية: `{prio}` | الرصيد: `{bal_str}` (أدنى: ${min_b:.2f})\n"
                ...
                for adapter in active_provs:
                    st = getattr(adapter, "status", "online")
                    st_icon = "🟢" if st == "online" else "🔴"
                    prio = getattr(adapter, "priority", 100)
                    markup.add(
                        types.InlineKeyboardButton(f"⚙️ {st_icon} إدارة: {adapter.name} (أولوية {prio})", callback_data=f"adm_custom_prov_{adapter.provider_id}")
                    )
