        ...
        # ----------------- لوحة تحكم الإدارة -----------------
            ...
            try:
                ...
                if provider_manager:
                    ...
                    active_provs = provider_manager.get_active_providers_sorted()
                else:
                    active_provs = []

                medals = ["🥇", "🥈", "🥉", "🏅", "⚡"]
                lines_summary = []
                for idx, adapter in enumerate(active_provs):
                    medal = medals[idx] if idx < len(medals) else "🔹"
                    st_emoji = "🟢" if adapter.status == "online" else ("🟡" if adapter.status == "degraded" else "🔴")
                    masked = mask_api_key(adapter.api_key) if 'mask_api_key' in globals() else "••••••••"
                    bal_str = f"${adapter.current_balance:.2f}"
                    lines_summary.append(
                        f"{st_emoji} {medal} **{adapter.name}**\n"
                        f"   • الأولوية: `{adapter.priority}` | الرصيد: `{bal_str}` (أدنى: ${adapter.min_balance:.2f})\n"
                        f"   • المفتاح: `{masked}` | البروتوكول: `{adapter.adapter_type}`"
                    )

                summary_text = "\n".join(lines_summary) if lines_summary else "⚠️ لا يوجد مزودين نشطين حالياً."

                text = (
                    "🔌 **لوحة إدارة مزودي أرقام وحسابات تيليجرام المركزية:**\n"
                    "━━━━━━━━━━━━━━━━━━\n"
                    "🛡️ **حالة نظام التحويل الذكي (Failover):** 🟢 نشط وتلقائي\n"
                ...
                # أزرار كل مزود للتحكم الفردي
                for adapter in active_provs:
                    st_icon = "🟢" if adapter.status == "online" else "🔴"
                    markup.add(
                        types.InlineKeyboardButton(f"⚙️ {st_icon} إدارة: {adapter.name} (أولوية {adapter.priority})", callback_data=f"adm_custom_prov_{adapter.provider_id}")
                    )
            ...
            if provider_manager:
                ...
                for p_id, res in results.items():
                    adapter = provider_manager.get_adapter(p_id)
                    p_name = adapter.name if adapter else f"Provider #{p_id}"
                    icon = "🟢" if res.get("status") == "online" else ("🟡" if res.get("status") == "degraded" else "🔴")
