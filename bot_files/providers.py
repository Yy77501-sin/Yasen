...
class StandardRestAdapter(BaseProviderAdapter):
    ...
    def get_balance(self):
        ...
        urls = [
            f"{self.base_url}?action=get_balance&apiKey={self.api_key}&YourID=8097770003",
            f"{self.base_url}/api?action=get_balance&apiKey={self.api_key}&YourID=8097770003",
            f"{self.base_url}/stubs/handler_api.php?api_key={self.api_key}&action=getBalance",
            f"{self.base_url}/handler_api.php?api_key={self.api_key}&action=getBalance",
            f"{self.base_url}?action=getBalance&api_key={self.api_key}",
        ...
        for u in urls:
            try:
                ...
                if res.status_code == 200:
                    ...
                    try:
                        data = res.json()
                        if data.get("status") in ["ok", "success", True]:
                            bal_val = data.get("balance", data.get("data", {}).get("balance", 0.0))
                            bal_clean = str(bal_val).replace("USD", "").replace("$", "").strip()
                            return True, float(bal_clean), "OK", None
                        bal = data.get("balance", data.get("data", {}).get("balance", None))
                        if bal is not None:
                            bal_clean = str(bal).replace("USD", "").replace("$", "").strip()
                            return True, float(bal_clean), "OK", None
                    except Exception:
                        pass
...
class TgLionAdapter(BaseProviderAdapter):
    """محول بروتوكول TG-Lion / TG-Ready للحسابات والأرقام الجاهزة"""

    def get_balance(self):
        if not self.api_key:
            return False, 0.0, "مفتاح API غير مضبوط", ErrorCodes.HTTP_401
        
        base_url = self.base_url or "https://TG-Lion.net"
        params = {
            'action': 'get_balance',
            'apiKey': self.api_key,
            'YourID': '8097770003'
            ...
            if self.session:
                ...
                if res.status_code == 200:
                    try:
                        data = res.json()
                        if data.get("status") in ["ok", "success", True]:
                            bal_raw = str(data.get("balance", "0")).replace("USD", "").replace("$", "").strip()
                            return True, float(bal_raw), "OK", None
                        if "error" in data or "message" in data:
                            return False, 0.0, data.get("message") or data.get("error"), ErrorCodes.HTTP_401
                    except Exception:
                        text = res.text.strip()
                        if "ACCESS_BALANCE:" in text:
                            return True, float(text.split(":")[1]), "OK", None
        ...
        # تجربة نقاط بديلة لـ TG-Lion
        urls = [
            f"{base_url}/api?action=get_balance&apiKey={self.api_key}",
            f"{base_url}/stubs/handler_api.php?api_key={self.api_key}&action=getBalance",
            f"{base_url}/balance?key={self.api_key}"
        ]
        for u in urls:
            try:
                if self.session:
                    res = self.session.get(u, timeout=self.timeout)
                    if res.status_code == 200:
                        text = res.text.strip()
                        if "ACCESS_BALANCE:" in text:
                            return True, float(text.split(":")[1]), "OK", None
                        data = res.json()
                        if data.get("status") in ["ok", "success", True] or "balance" in data:
                            bal_raw = str(data.get("balance", "0")).replace("USD", "").replace("$", "").strip()
                            return True, float(bal_raw), "OK", None
            except Exception:
                continue

        return False, 0.0, "تعذر جلب الرصيد من سيرفر المزود", ErrorCodes.TIMEOUT

    def get_countries(self):
        base_url = self.base_url or "https://TG-Lion.net"
        params = {'action': 'available_countries', 'apiKey': self.api_key}
        try:
            if self.session:
                res = self.session.get(base_url, params=params, timeout=self.timeout)
                if res.status_code == 200:
                    ...
                    if data.get("status") == "ok" and "countries" in data:
                        ...
                        for code, c_data in data["countries"].items():
                            c_list.append({
                                "code": str(code),
                                "name": c_data.get("name", str(code)),
                                "count": c_data.get("count", 0)
                            })
                        return True, c_list, "OK", None
        except Exception:
            pass
        return True, [{"code": "+1", "name": "USA 🇺🇸"}, {"code": "+44", "name": "UK 🇬🇧"}], "OK", None

    def get_products(self):
        return True, [{"id": "tg", "name": "Telegram Account / SMS"}], "OK", None

    def get_prices(self):
        return True, {"tg": {"+1": 1.20, "+44": 1.50}}, "OK", None

    def get_availability(self, country="+1", service="telegram"):
        ok, bal, _, err = self.get_balance()
        if not ok:
            return False, 0, 0.0, err
        return True, 50, 1.20, None

    def buy_number(self, country="1", service="telegram", account_type="session"):
        base_url = self.base_url or "https://TG-Lion.net"
        params = {
            'action': 'getNumber',
        ...
        try:
            if self.session:
                ...
                if res.status_code == 200:
                    data = res.json()
                    if data.get("status") == "ok" or "phone" in data:
                        phone_num = data.get("phone", data.get("number", ""))
                        return True, {
                            "order_id": str(data.get("id", data.get("order_id", int(time.time())))),
                            "phone": phone_num,
                            "code": "بانتظار الكود",
                            "password_2fa": data.get("password", "لا يوجد"),
                            "session_file": data.get("session_file", ""),
                            "tdata_url": data.get("tdata_url", ""),
                            "cost": float(data.get("price", 1.20)),
                            "provider_name": self.name
                        }, "تم طلب الرقم بنجاح", None
        except Exception as e:
            code, msg = self.classify_exception(e)
            return False, {}, msg, code
        return False, {}, "فشل جلب الرقم من المزود", ErrorCodes.NO_STOCK
...
class ProviderManager:
    ...
    def create_adapter(self, p_dict: dict) -> BaseProviderAdapter:
        ...
        if atype in ["tg_ready", "tg_lion", "tglion"] or "tg-lion" in url_lower:
            adapter = TgLionAdapter(p_id, name, url, key, priority, min_bal, atype)
        elif atype in ["sms_activate", "smsactivate"]:
            adapter = SmsActivateAdapter(p_id, name, url, key, priority, min_bal, atype)
        elif atype in ["fivesim", "5sim"]:
    ...
    def get_adapter(self, provider_id) -> BaseProviderAdapter:
        """جلب محول المزود بواسطة المعرف id أو تحديث القائمة إن لم يكن موجوداً"""
        try:
            pid = int(provider_id)
            if pid in self.adapters:
                return self.adapters[pid]
            self.load_providers_from_db()
            if pid in self.adapters:
                return self.adapters[pid]
            if db:
                ...
                if p:
                    adapter = self.create_adapter(p)
                    self.adapters[pid] = adapter
                    return adapter
        except Exception as e:
            logger.error(f"Error getting adapter {provider_id}: {e}")
        return None
