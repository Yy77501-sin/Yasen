...
class AppStore {
  ...
  loadAll() {
    ...
    this.config = {
      ...
      tgReadyProviders: {
        ...defaults.tgReadyProviders,
        ...(loaded.tgReadyProviders || {}),
      },
  ...
  // --- Ready Telegram Providers (Dynamic CRUD) ---
  getTgReadyProviders() {
    return this.config.tgReadyProviders || {};
  }

  getTgReadyProvider(providerKey) {
    return this.getTgReadyProviders()[providerKey] || null;
  }

  updateTgReadyProvider(providerKey, updates = {}) {
    const providers = { ...this.getTgReadyProviders() };
    if (!providers[providerKey]) {
      providers[providerKey] = { key: providerKey, name: providerKey, enabled: true };
    }
    providers[providerKey] = {
      ...providers[providerKey],
      ...updates,
    };
    this.config.tgReadyProviders = providers;
    this.persistAll();
    return providers[providerKey];
  }

  addTgReadyProvider({ name, baseUrl, apiKey }) {
    const providers = { ...this.getTgReadyProviders() };
    const key = `tg_custom_${Date.now()}`;
    const newProvider = {
      key,
      name: String(name || "موقع أرقام جاهزة").trim(),
      baseUrl: String(baseUrl || "https://api.grizzlysms.com/stubs/handler_api.php").trim(),
      apiKey: String(apiKey || "").trim(),
      enabled: true,
    };
  ...
  deleteTgReadyProvider(providerKey) {
    const providers = { ...this.getTgReadyProviders() };
    if (providers[providerKey]) {
      delete providers[providerKey];
      this.config.tgReadyProviders = providers;
  ...
  toggleTgReadyProvider(providerKey) {
    const providers = { ...this.getTgReadyProviders() };
    if (providers[providerKey]) {
