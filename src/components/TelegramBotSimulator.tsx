...
export const TelegramBotSimulator: React.FC<Props> = ({ userBalance, setUserBalance }) => {
  ...
  const [aiSupportQuery, setAiSupportQuery] = useState('');
  const [aiSupportAnswer, setAiSupportAnswer] = useState<string | null>(null);
  const [isAiAnswering, setIsAiAnswering] = useState(false);

  const handleAskAiSimulator = (customText?: string) => {
    const q = (customText || aiSupportQuery).trim().toLowerCase();
    if (!q) return;
    setIsAiAnswering(true);
    setAiSupportAnswer(null);

    setTimeout(() => {
      setIsAiAnswering(false);
      if (q.includes('كود') || q.includes('رمز') || q.includes('sms') || q.includes('ما وصل') || q.includes('تاخر') || q.includes('تأخر')) {
        setAiSupportAnswer('📱 **حل مشكلة تأخر أو عدم وصول كود SMS:**\n1. بعض الدول تتأخر لثوانٍ حسب ضغط الشبكة العالمية.\n2. **ميزة الاسترداد الفوري 100%:** إذا لم يصل الكود خلال دقيقتين، اضغط على زر [❌ إلغاء واسترجاع الرصيد] وسيتم إعادة المبلغ لمحفظتك فوراً دون أي خصم!\n3. يمكنك تجربة دولة أخرى أو سيرفر آخر.');
      } else if (q.includes('شحن') || q.includes('ايداع') || q.includes('إيداع') || q.includes('كريمي') || q.includes('فلوس') || q.includes('usdt') || q.includes('فودافون')) {
        setAiSupportAnswer('💳 **طرق شحن الرصيد المتاحة:**\n• 🇾🇪 اليمن: بنك الكريمي (حساب ون كاش/الكريمي) والحوالات المحلية.\n• 🇪🇬 مصر: فودافون كاش وإنستاباي.\n• 🇮🇶 العراق: آسيا حوالة وزين كاش.\n• 🌐 دولي: باير (Payeer) والعملات الرقمية USDT.\n📌 اضغط على [💳 شحن الرصيد] من القائمة الرئيسية وأرسل السند للتفعيل الفوري.');
      } else if (q.includes('جاهز') || q.includes('جلسة') || q.includes('session') || q.includes('tdata')) {
        setAiSupportAnswer('✈️ **شراء حسابات تيليجرام الجاهزة:**\nاضغط على [✈️ أرقام جاهزة] ثم اختر الدولة، وسيصلك الحساب مع كود الدخول وملف الجلسة (.session / Tdata) بضغطة زر دون انتظار كود SMS.');
      } else if (q.includes('مجاني') || q.includes('نقاط') || q.includes('إحالة') || q.includes('احالة') || q.includes('ربح')) {
        setAiSupportAnswer('🎁 **الرصيد المجاني:**\nاضغط على زر [🎁 رصيد مجاني] وانسخ رابط الدعوة الخاص بك، وستربح +0.10 $ عن كل مستخدم جديد ينضم للبوت عبر رابطك.');
      } else if (q.includes('رشق') || q.includes('متابعين') || q.includes('مشاهدات') || q.includes('اعضاء') || q.includes('الملك')) {
        setAiSupportAnswer('👑 **خدمات الرشق والمتابعين (نظام الملك):**\nيوفر البوت رشق أعضاء تيليجرام وانستغرام وتيك توك بأسعار تبدأ من 0.000271 $ للعضو الواحد مع سرعة تنفيذ فورية.');
      } else {
        setAiSupportAnswer('🤖 أهلاً بك! يمكنك تفعيل الأرقام لأي تطبيق عبر [📱 شراء الأرقام]، أو شحن محفظتك عبر [💳 شحن الرصيد]. وفي حال لم يصلك كود أي رقم، يمكنك إلغاؤه واسترداد رصيدك بالكامل فوراً. للتواصل المباشر مع الإدارة اضغط [💬 الدعم الفني].');
      }
    }, 600);
  };
            ...
            <div className="bg-[#182533] text-stone-100 rounded-2xl rounded-bl-xs p-4 text-xs max-w-[95%] sm:max-w-[90%] shadow-md border border-stone-800/80 space-y-3.5 leading-relaxed">
              ...
              {currentScreen === 'main' && (
                ...
                >
                  ...
                  {/* Inline Buttons Grid matching user original buttons with image layout */}
                  <div className="space-y-1.5 pt-1">
                    {/* Row 1: شراء الأرقام */}
                    <button
                      onClick={() => setCurrentScreen('buy_apps')}
                      className="w-full bg-gradient-to-r from-sky-600/30 via-sky-500/20 to-sky-600/30 hover:from-sky-600/40 hover:to-sky-600/40 active:scale-98 text-sky-200 p-2.5 rounded-xl font-bold text-xs text-center border border-sky-500/40 transition-all flex items-center justify-center gap-1.5 shadow-2xs"
                    >
                      <span className="text-base">📱</span>
                      <span>شراء الأرقام</span>
                    </button>

                    {/* Row 2: أرقام جاهزة | خدمات الرشق */}
                    <div className="grid grid-cols-2 gap-1.5">
                      ...
                      >
                        <span className="text-base">👑</span>
                        <span>خدمات الرشق</span>
                      </button>
                    </div>

                    {/* Row 3: عروض واتساب | شحن الرصيد */}
                    <div className="grid grid-cols-2 gap-1.5">
                      <button
                        onClick={() => {
                          const wa = MAIN_APPS_SIM.find(a => a.id === 'whatsapp');
                          if (wa) setSelectedApp(wa);
                          setCurrentScreen('buy_servers');
                        }}
                        className="bg-[#242f3d] hover:bg-[#2e3b4d] active:scale-98 text-white p-2.5 rounded-xl font-bold text-xs text-center border border-stone-700/60 transition-all flex items-center justify-center gap-1 shadow-2xs"
                      >
                        <span className="text-base">💬</span>
                        <span>عروض واتساب</span>
                      </button>
                      <button
                    ...
                    {/* Row 4: تحويل الرصيد | حسابي ومحفظتي */}
                    <div className="grid grid-cols-2 gap-1.5">
                      <button
                      ...
                      >
                        <span className="text-base">🪪</span>
                        <span>حسابي ومحفظتي</span>
                      </button>
                    </div>

                    {/* Row 5: الدعم الذكي (AI) | رصيد مجاني */}
                    <div className="grid grid-cols-2 gap-1.5">
                      <button
                        onClick={() => {
                          setAiSupportAnswer(null);
                          setAiSupportQuery('');
                          setCurrentScreen('ai_support');
                        }}
                        className="bg-linear-to-r from-sky-900/60 to-indigo-900/60 hover:from-sky-800/80 hover:to-indigo-800/80 active:scale-98 text-sky-200 p-2.5 rounded-xl font-bold text-xs text-center border border-sky-500/40 transition-all flex items-center justify-center gap-1 shadow-2xs"
                      >
                        <span className="text-base">🤖</span>
                    ...
                    {/* Row 6: السيرفرات | تعليمات واستخدام */}
                    <div className="grid grid-cols-2 gap-1.5">
                      <button
                      ...
                      <button
                        onClick={() => alert('📚 تعليمات الاستخدام:\n1. اختر التطبيق والدولة والسيرفر\n2. استلم الكود وانسخه تلقائياً فور وصوله\n3. في حال عدم وصول الكود يتم استرداد المبلغ فوراً.')}
                        className="bg-[#242f3d] hover:bg-[#2e3b4d] active:scale-98 text-white p-2.5 rounded-xl font-bold text-xs text-center border border-stone-700/60 transition-all flex items-center justify-center gap-1 shadow-2xs"
                      >
                        <span className="text-base">📚</span>
                        <span>تعليمات واستخدام</span>
                      </button>
                    </div>

                    {/* Row 7: القناة الرسمية | الدعم الفني */}
                    <div className="grid grid-cols-2 gap-1.5">
                      <button
                        onClick={() => alert('📢 القناة الرسمية للبوت: https://t.me/YasNumChannel')}
                        className="bg-[#242f3d] hover:bg-[#2e3b4d] active:scale-98 text-white p-2.5 rounded-xl font-bold text-xs text-center border border-stone-700/60 transition-all flex items-center justify-center gap-1 shadow-2xs"
                      >
                        <span className="text-base">📢</span>
              ...
              {/* Screen: AI SUPPORT ASSISTANT */}
              {currentScreen === 'ai_support' && (
                <div className="space-y-3">
                  <div className="bg-linear-to-r from-sky-950/60 to-indigo-950/60 border border-sky-600/40 p-3 rounded-xl space-y-2">
                    <p className="font-bold text-sm text-sky-300 flex items-center gap-1.5">
                      <span>🤖</span>
                      <span>المساعد الذكي للدعم الفوري (AI)</span>
                    </p>
                    <p className="text-stone-300 text-xs leading-relaxed">
                      أهلاً بك! أنا مساعدك الذكي لحل جميع مشاكلك والإجابة على استفساراتك حول البوت فورياً (24/7).
                    </p>
                  </div>

                  {/* Quick FAQ Questions */}
                  <div className="space-y-1.5">
                    <p className="text-stone-400 text-[11px] font-bold">💡 أسئلة واستفسارات سريعة:</p>
                    <div className="grid grid-cols-1 gap-1">
                      <button
                        onClick={() => handleAskAiSimulator('كود التحقق لم يصل وكيف استرجع فلوسي')}
                        className="w-full text-right bg-[#242f3d] hover:bg-[#2e3b4d] text-stone-200 p-2 rounded-lg text-xs border border-stone-700/60 flex items-center justify-between"
                      >
                        <span>📱 لم يصلني كود الـ SMS وكيف أسترجع رصيدي؟</span>
                        <span className="text-sky-400 text-xs">←</span>
                      </button>
                      <button
                        onClick={() => handleAskAiSimulator('طرق شحن الرصيد في اليمن ومصر والعراق')}
                        className="w-full text-right bg-[#242f3d] hover:bg-[#2e3b4d] text-stone-200 p-2 rounded-lg text-xs border border-stone-700/60 flex items-center justify-between"
                      >
                        <span>💳 ما هي طرق شحن الرصيد والإيداع المتاحة؟</span>
                        <span className="text-sky-400 text-xs">←</span>
                      </button>
                      <button
                        onClick={() => handleAskAiSimulator('شرح شراء حسابات تيليجرام الجاهزة')}
                        className="w-full text-right bg-[#242f3d] hover:bg-[#2e3b4d] text-stone-200 p-2 rounded-lg text-xs border border-stone-700/60 flex items-center justify-between"
                      >
                        <span>✈️ كيف أشتري وأسجل بحساب تيليجرام جاهز؟</span>
                        <span className="text-sky-400 text-xs">←</span>
                      </button>
                      <button
                        onClick={() => handleAskAiSimulator('كيف اربح رصيد مجاني من رابط الدعوة')}
                        className="w-full text-right bg-[#242f3d] hover:bg-[#2e3b4d] text-stone-200 p-2 rounded-lg text-xs border border-stone-700/60 flex items-center justify-between"
                      >
                        <span>🎁 كيف أربح رصيداً مجانياً من رابط الإحالة؟</span>
                        <span className="text-sky-400 text-xs">←</span>
                      </button>
                    </div>
                  </div>
                  ...
                  <form
                    onSubmit={(e) => {
                      e.preventDefault();
                      handleAskAiSimulator();
                    }}
                    className="space-y-1.5"
                  >
                    <label className="text-stone-300 text-xs font-bold block">✍️ أو اكتب مشكلتك/سؤالك هنا:</label>
                    <div className="flex gap-1.5">
                      <input
                        type="text"
                        value={aiSupportQuery}
                        onChange={(e) => setAiSupportQuery(e.target.value)}
                        placeholder="مثال: كيف أحول رصيد لصديقي؟ أو اشتريت رقم..."
                        className="grow bg-[#242f3d] border border-stone-700 text-white px-3 py-1.5 rounded-xl text-xs outline-hidden focus:border-sky-500"
                      />
                      <button
                        type="submit"
                        disabled={isAiAnswering || !aiSupportQuery.trim()}
                        className="bg-sky-600 hover:bg-sky-500 disabled:opacity-50 text-white px-3 py-1.5 rounded-xl font-bold text-xs shrink-0"
                      >
                        {isAiAnswering ? '⏳' : 'إرسال'}
                      </button>
                    </div>
                  ...
                  {/* AI Response Display */}
                  {isAiAnswering && (
                    <div className="bg-sky-950/40 border border-sky-700/40 p-3 rounded-xl text-xs text-sky-200 animate-pulse flex items-center gap-2">
                      <span className="animate-spin text-sm">⏳</span>
                      <span>جارٍ التفكير وتحليل المشكلة بواسطة الذكاء الاصطناعي...</span>
                    </div>
                  )}

                  {aiSupportAnswer && !isAiAnswering && (
                    <div className="bg-[#1c242f] border border-sky-600/50 p-3 rounded-xl text-xs text-stone-200 space-y-2">
                      <p className="font-bold text-sky-300 flex items-center gap-1">
                        <span>🤖</span>
                        <span>إجابة المساعد الذكي:</span>
                      </p>
                      <div className="whitespace-pre-line text-stone-200 leading-relaxed text-[11.5px]">
                        {aiSupportAnswer}
                      </div>
                    </div>
                  )}
                  ...
                  <div className="grid grid-cols-2 gap-1.5 pt-1">
                    <button
                      onClick={() => setCurrentScreen('buy_apps')}
                      className="bg-[#242f3d] hover:bg-[#2e3b4d] text-white py-2 rounded-xl text-xs font-bold border border-stone-700/60"
                    >
                      📱 شراء رقم
                    </button>
                    <button
                ...
                <div className="space-y-3">
                  ...
                  <div className="space-y-1.5 pt-1">
                    <div className="grid grid-cols-2 gap-1.5">
                      ...
                      >
                        <span className="text-base">🔑</span>
                        <span>إدارة المزودين والمفاتيح</span>
                        <span className="text-[10px] text-sky-300 font-normal">سيرفرات ومفاتيح الـ API</span>
                      </button>
              ...
              {currentScreen === 'admin_servers' && (
                <div className="space-y-3">
                  ...
                  <div className="flex gap-2">
                    <button
                      onClick={() => {
                        const name = prompt('أدخل اسم المزود الجديد (مثال: SMS-Activate):');
                        if (!name) return;
                        const site = prompt('أدخل رابط الموقع (مثال: sms-activate.org):');
                        const key = prompt('أدخل مفتاح API:');
                        if (name && site && key) {
                          setAdminServers(prev => [
                            ...prev,
                            { id: Date.now().toString(), name, site, key, balance: '0.00 $', enabled: true }
                          ]);
                          alert(`✅ تم إضافة المزود "${name}" بنجاح وتفعيله في قائمة السيرفرات!`);
                        }
                      }}
                      className="w-full bg-emerald-800 hover:bg-emerald-700 text-white py-2 rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-1"
                    >
                      <span>➕</span>
                      <span>إضافة مزود أرقام وسيرفر جديد</span>
                    </button>
                  </div>
              ...
              {currentScreen === 'admin_check_providers' && (
                <div className="space-y-3">
                  ...
                  <div className="flex gap-2">
                    <button
                      onClick={() => setCurrentScreen('admin_servers')}
                      className="w-full bg-[#242f3d] hover:bg-[#2e3b4d] border border-sky-600/60 text-sky-200 py-2 rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-1.5"
                    >
                      <span>🔑</span>
                      <span>إدارة المزودين والمفاتيح (تعديل الـ API والتحكم)</span>
                    </button>
                  </div>
